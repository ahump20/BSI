 ...
 */
import { ScheduleOptimizer } from '../../../lib/analytics/baseball/schedule-optimizer';
import { ConferenceStrengthModel } from '../../../lib/analytics/baseball/conference-strength-model';
// ============================================================================
// API Handler
// ============================================================================
 ...
 */
async function runOptimization(request, env) {
    // Fetch team data
    ...
    const simulation = ScheduleOptimizer.runMonteCarloSimulation(team.schedule, teamMetrics, opponentMetrics, request.iterations);
    // Generate what-if scenarios (optional)
    let whatIfScenarios;
    if (request.scenarios) {
        whatIfScenarios = ScheduleOptimizer.generateWhatIfScenarios(simulation, team.schedule, teamMetrics);
    }
    // Generate optimization recommendations (optional)
    let optimization;
    if (request.optimize) {
        optimization = ScheduleOptimizer.optimizeSchedule(simulation, team.schedule, teamMetrics, opponentMetrics);
    }
    // Calculate conference strength
    const conferenceStrengths = ConferenceStrengthModel.calculateConferenceStrength(allTeams.filter(t => t.conference === team.schedule.conference), allTeams);
    const conferenceStrength = conferenceStrengths.find(c => c.conference === team.schedule.conference);
    return {
        simulation,
        whatIfScenarios,
        optimization,
 ...
 */
async function fetchTeamData(teamId, env) {
    // Fetch team info
  ...
  `).bind(teamResult.id, teamResult.id, teamResult.id, teamResult.id, teamResult.id, now).all();
    ...
    const remainingGames = scheduleResults.results.map((row) => {
  ...
  `).bind(teamResult.id, teamResult.id, teamResult.id).all();
    const opponents = opponentsResults.success
        ? opponentsResults.results.map((row) => row.opponent_id)
        : [];
    const schedule = {
    ...
    const record = {
        ...
        wins: teamResult.wins,
        losses: teamResult.losses,
        conferenceWins: teamResult.conference_wins,
        conferenceLosses: teamResult.conference_losses,
        homeWins: teamResult.home_wins,
        homeLosses: teamResult.home_losses,
        awayWins: teamResult.away_wins,
        awayLosses: teamResult.away_losses,
        neutralWins: teamResult.neutral_wins,
        neutralLosses: teamResult.neutral_losses,
        runsScored: teamResult.runs_scored,
        runsAllowed: teamResult.runs_allowed,
        opponents
    };
 ...
 */
async function fetchAllTeams(env) {
  ...
  `).all();
    ...
    `).bind(row.id, row.id, row.id).all();
        const opponents = opponentsResults.success
            ? opponentsResults.results.map((opp) => opp.opponent_id)
            : [];
        teams.push({
            teamId: row.id,
            teamName: row.name,
            ...
            wins: row.wins,
            losses: row.losses,
            conferenceWins: row.conference_wins,
            conferenceLosses: row.conference_losses,
            homeWins: row.home_wins,
            homeLosses: row.home_losses,
            awayWins: row.away_wins,
            awayLosses: row.away_losses,
            neutralWins: row.neutral_wins,
            neutralLosses: row.neutral_losses,
