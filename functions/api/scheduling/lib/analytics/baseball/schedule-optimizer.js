...
export class ScheduleOptimizer {
    ...
    static runMonteCarloSimulation(schedule, teamMetrics, opponentMetrics, iterations = 10000) {
        ...
        const confChampProb = this.estimateConferenceChampionshipProb(meanWins, totalGames, schedule.currentRecord.wins, schedule.currentRecord.losses);
    ...
    static generateWhatIfScenarios(baseSimulation, schedule, teamMetrics) {
        ...
        scenarios.push(this.createScenario('Win Out', 'Win all remaining games', schedule.remainingGames.map(g => ({ gameId: g.gameId, assumedResult: 'win' })), schedule, teamMetrics));
        // Scenario 2: Lose all remaining games
        scenarios.push(this.createScenario('Worst Case', 'Lose all remaining games', schedule.remainingGames.map(g => ({ gameId: g.gameId, assumedResult: 'loss' })), schedule, teamMetrics));
        ...
        scenarios.push(this.createScenario('Win Key Games', 'Win all games against key opponents', keyGames.map(g => ({ gameId: g.gameId, assumedResult: 'win' })), schedule, teamMetrics));
        ...
        scenarios.push(this.createScenario('Split Remaining', 'Win 50% of remaining games', splitGames, schedule, teamMetrics));
        return scenarios;
    }
    /**
    ...
    static createScenario(name, description, outcomes, schedule, teamMetrics) {
        ...
        const baseNCAAProb = this.estimateNCAAProb(schedule.currentRecord.wins, schedule.currentRecord.wins + schedule.currentRecord.losses, teamMetrics.rpi.rpi);
        const scenarioNCAAProb = this.estimateNCAAProb(projectedWins, totalGames, teamMetrics.rpi.rpi + rpiChange);
    ...
    static optimizeSchedule(simulation, schedule, teamMetrics, opponentMetrics) {
        ...
        const recommendations = this.generateRecommendations(simulation, schedule, teamMetrics, keyGames);
    ...
    static generateRecommendations(simulation, schedule, teamMetrics, keyGames) {
        ...
        const closeGames = simulation.remainingGameProbabilities.filter(g => g.winProbability >= 0.40 && g.winProbability <= 0.60);
        ...
        const qualityWinOpps = simulation.remainingGameProbabilities.filter(g => g.winProbability >= 0.30 // Winnable games
        ).length;
        if (qualityWinOpps > 3) {
            recommendations.push({
                priority: 2,
        ...
        const mustWinGames = simulation.remainingGameProbabilities.filter(g => g.winProbability >= 0.70).length;
        if (mustWinGames > 0) {
            recommendations.push({
                priority: 3,
        ...
        if (simulation.ncaaTournamentProbability < 0.70) {
            const winsNeeded = Math.ceil((schedule.currentRecord.wins + schedule.remainingGames.length) * 0.55) - schedule.currentRecord.wins;
            recommendations.push({
                priority: 4,
