/**
 * Blaze Sports Intel - MLB AI Game Previews & Recaps
 * Fetches MLB games and generates AI-powered previews/recaps
 */
...
interface ESPNGame {
  ...
  competitions: Array<{
    competitors: Array<{
      ...
      probables?: Array<{ athlete: { displayName: string } }>;
    }>;
    venue?: {
      fullName: string;
    };
...
const ESPN_MLB_API = 'https://site.api.espn.com/apis/site/v2/sports/baseball/mlb';
...
async function generatePreview(env: Env, game: ESPNGame): Promise<string> {
  ...
  const prompt = `You are an MLB analyst writing a game preview. Write a 2-paragraph preview.
...
1. The pitching matchup and what each starter brings
2. Key hitters to watch and recent team trends

Keep it under 150 words. Be analytical like a seasoned baseball writer. Focus on the matchup dynamics.`;
  ...
  try {
    ...
    return `${awayTeam?.team.displayName} heads to ${venue} to face the ${homeTeam?.team.displayName}. First pitch details available closer to game time.`;
  }
}
...
async function generateRecap(env: Env, game: ESPNGame): Promise<string> {
  ...
  const prompt = `You are an MLB analyst writing a game recap. Write a 2-paragraph summary.
...
1. The decisive moments—key hits, pitching performances, or defensive plays
2. What this game means for both teams' standings and momentum

Keep it under 150 words. Write like you're on Baseball Tonight—analytical and direct.`;
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // Check cache first
    const cacheKey = `mlb:games:${date}:${limit}`;
    if (env.SPORTS_CACHE) {
    ...
    const response = await fetch(`${ESPN_MLB_API}/scoreboard?dates=${date}`);
    if (!response.ok) {
    ...
    const processedGames = await Promise.all(
      games.slice(0, limit).map(async (game) => {
        ...
        const contentCacheKey = `mlb:${contentType}:${game.id}`;
        ...
        return {
          ...
          inningState: game.status.displayClock,
          homeTeam: {
          ...
          aiContent: isLive ? 'Game in progress—live updates coming.' : aiContent,
        };
      })
    );
    ...
    // Cache for 5 minutes (shorter for MLB during season)
    if (env.SPORTS_CACHE) {
