/**
 * CBB Game Detail API Endpoint
 *
 * GET /api/cbb/game/:gameId
 */
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const summary = await adapter.getGameSummary('ncaab', gameId);
    ...
    const response = {
      game: {
        ...
        status: {
          ...
          period: (gameData.sportData as any)?.period,
      ...
      plays: summary.plays,
      meta: {
        dataSource: 'ESPN College Basketball',
    ...
    console.error('CBB game error:', error);
    return new Response(
      JSON.stringify({
...
function extractLeaders(leaders: any): any {
  ...
  return {
    points: extractLeaderCategory(leaders, 'points'),
    rebounds: extractLeaderCategory(leaders, 'rebounds'),
    assists: extractLeaderCategory(leaders, 'assists'),
  };
}
