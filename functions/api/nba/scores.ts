/**
 * NBA Live Scores API - Real SportsDataIO Data
 * Live and completed game scores with real-time updates
 *
 * Endpoints:
 * - GET /api/nba/scores - Today's games
 * - GET /api/nba/scores?date=2025-NOV-13 - Specific date (YYYY-MMM-DD format)
 *
...
export const onRequest: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    const response = await adapter.getNBAScores(date);
