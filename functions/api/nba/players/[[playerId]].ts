/**
 * NBA Player Profile API Endpoint
 *
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const playerStats = await adapter.getPlayerStats('nba', playerId);
    ...
    const response = {
      ...
      stats: {
        season,
        stats: extractNBAStats(playerStats),
      },
      meta: {
        dataSource: 'ESPN API',
    ...
    console.error('NBA player profile error:', error);
    return new Response(
      JSON.stringify({
...
function extractNBAStats(playerStats: any): Record<string, number | string> {
