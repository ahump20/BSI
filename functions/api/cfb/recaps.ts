 ...
 * CFB Recaps API
 *
 * Returns a list of game recap articles from Coded Content.
 * First checks KV cache, then falls back to D1.
 *
 * GET /api/cfb/recaps?limit=10&offset=0
 */
...
const CACHE_KEY = 'cfb:recaps:latest';
  ...
  try {
    ...
    const countResult = await env.DB.prepare(
      `SELECT COUNT(*) as total FROM coded_content_articles
       WHERE league = 'cfb' AND content_type = 'recap'`
    ).first<{ total: number }>();
    ...
    console.error('CFB Recaps API error:', error);
    return new Response(
      JSON.stringify({
