/**
 * NCAA Football Live Scores API - Real SportsDataIO Data
 * Live and completed game scores with real-time updates
 *
 * Endpoints:
 * - GET /api/ncaa/football-scores - Current week's games
 * - GET /api/ncaa/football-scores?week=5 - Specific week
 * - GET /api/ncaa/football-scores?season=2024&week=5 - Historical week
 *
...
export const onRequest: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    const response = await adapter.getNCAAFScores(season, week);

    if (!response.success || !response.data) {
      return new Response(
        JSON.stringify({
          error: 'Failed to fetch NCAA Football scores',
    ...
    console.error('NCAA Football Scores Error:', error);
    return new Response(
      JSON.stringify({
