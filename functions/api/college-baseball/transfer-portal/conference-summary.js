...
export async function onRequest(context) {
  ...
  try {
    ...
    const inboundResults = await db
      .prepare(inboundQuery)
      .bind(...(conference ? [conference] : []))
      .all();
    ...
    const topHittersQuery = `
      ...
      LIMIT 25
    `;
    const topHitters = await db
      .prepare(topHittersQuery)
      .bind(...(conference ? [conference] : []))
      .all();
    ...
    const topArmsQuery = `
      ...
      LIMIT 25
    `;
    const topArms = await db
      .prepare(topArmsQuery)
      .bind(...(conference ? [conference] : []))
      .all();
    ...
    const conferenceSummary = Array.from(conferenceMap.values()).sort((a, b) => b.net - a.net);
