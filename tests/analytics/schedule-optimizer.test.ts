...
const mockTeamMetrics = {
  ...
  isr: {
    ...
    conference: 'SEC',
    isr: 0.7800,
    offensiveRating: 0.8200,
    defensiveRating: 0.7600,
    recentForm: 0.7500
  } as ISRCalculation
};
...
const mockOpponentMetrics = new Map([
  ['tennessee', {
    rpi: { teamId: 'tennessee', teamName: 'Tennessee', conference: 'SEC', rpi: 0.6500, wp: 0.8200, owp: 0.5600, oowp: 0.5400 } as RPICalculation,
    sos: { teamId: 'tennessee', teamName: 'Tennessee', conference: 'SEC', sos: 0.7000, opponentAvgRPI: 0.5600, qualityWins: 15, badLosses: 0 } as SOSCalculation,
    isr: { teamId: 'tennessee', teamName: 'Tennessee', conference: 'SEC', isr: 0.8400, offensiveRating: 0.8800, defensiveRating: 0.8200, recentForm: 0.8200 } as ISRCalculation
  }],
  ['vanderbilt', {
    rpi: { teamId: 'vanderbilt', teamName: 'Vanderbilt', conference: 'SEC', rpi: 0.5800, wp: 0.6100, owp: 0.5400, oowp: 0.5200 } as RPICalculation,
    sos: { teamId: 'vanderbilt', teamName: 'Vanderbilt', conference: 'SEC', sos: 0.6500, opponentAvgRPI: 0.5400, qualityWins: 8, badLosses: 2 } as SOSCalculation,
    isr: { teamId: 'vanderbilt', teamName: 'Vanderbilt', conference: 'SEC', isr: 0.6800, offensiveRating: 0.7200, defensiveRating: 0.6800, recentForm: 0.6100 } as ISRCalculation
  }],
  ['florida', {
    rpi: { teamId: 'florida', teamName: 'Florida', conference: 'SEC', rpi: 0.5200, wp: 0.5500, owp: 0.5300, oowp: 0.5100 } as RPICalculation,
    sos: { teamId: 'florida', teamName: 'Florida', conference: 'SEC', sos: 0.6200, opponentAvgRPI: 0.5300, qualityWins: 5, badLosses: 4 } as SOSCalculation,
    isr: { teamId: 'florida', teamName: 'Florida', conference: 'SEC', isr: 0.6100, offensiveRating: 0.6400, defensiveRating: 0.6200, recentForm: 0.5500 } as ISRCalculation
  }]
]);
...
describe('ScheduleOptimizer - Monte Carlo Simulations', () => {
  ...
  it('should produce projected record within valid range', () => {
    const totalGames = mockSchedule.currentRecord.wins + mockSchedule.currentRecord.losses + mockSchedule.remainingGames.length;
    ...
    expect(simulation.projectedRecord.losses).toBeGreaterThanOrEqual(mockSchedule.currentRecord.losses);
    expect(simulation.projectedRecord.losses).toBeLessThanOrEqual(totalGames - mockSchedule.currentRecord.wins);
  });
  ...
  it('should produce 95% confidence interval', () => {
    ...
    expect(simulation.confidenceInterval.winsLower).toBeLessThanOrEqual(simulation.projectedRecord.wins);
    expect(simulation.confidenceInterval.winsUpper).toBeGreaterThanOrEqual(simulation.projectedRecord.wins);
    expect(simulation.confidenceInterval.lossesLower).toBeLessThanOrEqual(simulation.projectedRecord.losses);
    expect(simulation.confidenceInterval.lossesUpper).toBeGreaterThanOrEqual(simulation.projectedRecord.losses);
  });

  it('should have confidence interval width proportional to uncertainty', () => {
    const intervalWidth = simulation.confidenceInterval.winsUpper - simulation.confidenceInterval.winsLower;
  ...
  it('should estimate NCAA tournament probability', () => {
    ...
    expect(simulation.ncaaTournamentProbability).toBeGreaterThan(0.70);
  });
  ...
  it('should estimate NCAA national seed probability', () => {
    ...
    expect(simulation.ncaaSeedProbability).toBeLessThanOrEqual(simulation.ncaaTournamentProbability);
  });
...
describe('ScheduleOptimizer - Log5 Win Probability', () => {
  it('should calculate win probability using Log5 formula', () => {
    ...
    expect(winProb).toBeGreaterThan(0.30);
    expect(winProb).toBeLessThan(0.50);
  });
  ...
  it('should apply home field advantage (6% boost)', () => {
    ...
    expect(homeProb - neutralProb).toBeLessThan(0.10); // At most 10% boost
  });
  ...
  it('should apply away field disadvantage (6% penalty)', () => {
    ...
    expect(neutralProb - awayProb).toBeLessThan(0.10); // At most 10% penalty
  });

  it('should clamp probabilities between 5% and 95%', () => {
    // Create extreme mismatch scenario
    const strongTeam = {
      rpi: { ...mockTeamMetrics.rpi, rpi: 0.9500 },
      sos: mockTeamMetrics.sos,
      isr: { ...mockTeamMetrics.isr, isr: 0.9500 }
    };

    const weakOpponent = {
      rpi: { teamId: 'weak', teamName: 'Weak Team', conference: 'Weak', rpi: 0.1000, wp: 0.1000, owp: 0.3000, oowp: 0.3000 } as RPICalculation,
      sos: { teamId: 'weak', teamName: 'Weak Team', conference: 'Weak', sos: 0.2000, opponentAvgRPI: 0.3000, qualityWins: 0, badLosses: 10 } as SOSCalculation,
      isr: { teamId: 'weak', teamName: 'Weak Team', conference: 'Weak', isr: 0.1000, offensiveRating: 0.1000, defensiveRating: 0.1500, recentForm: 0.1000 } as ISRCalculation
    };
...
describe('ScheduleOptimizer - What-If Scenarios', () => {
  ...
  it('should calculate "Split Remaining" scenario correctly', () => {
    ...
    const expectedWins = mockSchedule.currentRecord.wins + Math.floor(mockSchedule.remainingGames.length / 2);
    const expectedLosses = mockSchedule.currentRecord.losses + Math.ceil(mockSchedule.remainingGames.length / 2);
  ...
  it('should show "Win Out" has highest NCAA probability boost', () => {
    ...
    expect(winOut).toBeDefined();
    expect(worstCase).toBeDefined();
    expect(winOut!.ncaaTournamentProbabilityChange).toBeGreaterThan(worstCase!.ncaaTournamentProbabilityChange);
  });
});
...
describe('ScheduleOptimizer - Statistical Validation', () => {
  it('should produce consistent results with same seed', () => {
    ...
    expect(Math.abs(simulation1.projectedRecord.wins - simulation2.projectedRecord.wins)).toBeLessThanOrEqual(1);
  });
  ...
  it('should converge with more iterations', () => {
    ...
    const width1k = simulation1k.confidenceInterval.winsUpper - simulation1k.confidenceInterval.winsLower;
    const width10k = simulation10k.confidenceInterval.winsUpper - simulation10k.confidenceInterval.winsLower;
  ...
  it('should respect binomial distribution properties', () => {
    ...
    const expectedWins = mockSchedule.currentRecord.wins +
...
describe('ScheduleOptimizer - Edge Cases', () => {
  ...
  it('should handle missing opponent metrics', () => {
    ...
    expect(simulation.remainingGameProbabilities[0].winProbability).toBe(0.50);
  });
  ...
  it('should handle extreme iteration counts', () => {
    ...
    expect(simulation100.projectedRecord.wins).toBeGreaterThanOrEqual(mockSchedule.currentRecord.wins);
