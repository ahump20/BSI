...
async function ingestStandings(env: Env): Promise<{ source: string; conferences: number; error?: string }> {
  ...
  if (env.RAPIDAPI_KEY) {
    ...
    for (const conf of CONFERENCES) {
      const result = await safeFetch<Record<string, unknown>>(
        `${HIGHLIGHTLY_BASE}/standings?league=NCAA&conference=${encodeURIComponent(conf)}`,
        highlightlyHeaders(env.RAPIDAPI_KEY)
      );
      if (result.ok && result.data) {
        await env.KV.put(`cb:standings:${conf}`, JSON.stringify(result.data), { expirationTtl: TTL.standings });
        hlSuccess++;
      }
    }
  ...
  if (result.ok && result.data) {
    ...
    for (const conf of CONFERENCES) {
      ...
      if (match) {
        await env.KV.put(`cb:standings:${conf}`, JSON.stringify([match]), { expirationTtl: TTL.standings });
        written++;
      }
    }

    // Also write NCAA-level (all)
    await env.KV.put('cb:standings:NCAA', JSON.stringify(children), { expirationTtl: TTL.standings });
...
export default {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/status') {
      ...
      const standingsFresh = !!(await env.KV.get('cb:standings:NCAA', 'text'));
