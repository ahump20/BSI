...
async function initializeDatabase(db: D1Database): Promise<void> {
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_leaderboard_category_score ON leaderboard(category, score DESC)`);
}
...
const CHARACTERS: Character[] = [
  {
    ...
    ability: { name: 'Thunder Steal', description: 'Steals bases 75% faster with 90% success rate', cooldown: 2 },
  ...
  },
  {
    ...
    ability: { name: 'Launch Pad', description: 'Next home run goes 50% farther for bonus points', cooldown: 3 },
  ...
  },
  {
    ...
    ability: { name: 'Gold Glove Dive', description: 'Can catch any ball within the field for one play', cooldown: 4 },
  ...
  },
  {
    ...
    ability: { name: 'Phantom Ball', description: 'Next pitch becomes nearly invisible for 2 seconds', cooldown: 3 },
  ...
  },
  {
    ...
    ability: { name: 'Clutch Mode', description: 'All stats increase by 20% in final inning', cooldown: 0 },
  ...
  },
  {
    ...
    ability: { name: 'Wind Sprint', description: 'Moves twice as fast for 10 seconds', cooldown: 4 },
  ...
  },
  {
    ...
    ability: { name: 'Laser Throw', description: 'Throws out any base stealer for one play', cooldown: 3 },
  ...
  },
  {
    ...
    ability: { name: 'Lucky Bounce', description: '15% chance any fair ball becomes a hit', cooldown: 0 },
  ...
  },
  {
    ...
    ability: { name: 'Ice Cold', description: 'No pressure penalties in high-leverage situations', cooldown: 0 },
  ...
  },
  {
    ...
    ability: { name: 'Iron Wall', description: 'Prevents all ground balls from getting through infield', cooldown: 5 },
  ...
  },
  {
    ...
    ability: { name: 'Creative Play', description: 'Can turn singles into doubles with smart baserunning', cooldown: 2 },
  ...
  },
  {
    ...
    ability: { name: 'Hot Streak', description: 'Each consecutive hit increases power by 15%', cooldown: 0 },
...
const STADIUMS: Stadium[] = [
  {
    ...
    features: [
      { type: 'tree', name: 'Oak Tree', position: { x: -150, y: 200 }, effect: 'Balls that hit the tree drop straight down for an automatic double' },
      { type: 'obstacle', name: 'Tire Swing', position: { x: 100, y: 190 }, effect: 'Hit the tire for bonus points!' },
      { type: 'bonus', name: 'Hill Slope', position: { x: 0, y: 210 }, effect: 'Ground balls roll faster uphill, slower downhill' },
    ],
  ...
  },
  {
    ...
    features: [
      { type: 'obstacle', name: 'Cactus Garden', position: { x: -180, y: 180 }, effect: 'Balls landing in cactus are ground rule doubles' },
      { type: 'bonus', name: 'Desert Wind', position: { x: 0, y: 0 }, effect: 'Strong crosswind affects all fly balls' },
      { type: 'feature', name: 'Lizard Rock', position: { x: 50, y: 150 }, effect: 'Hit the rock for a lucky bounce' },
    ],
  ...
  },
  {
    ...
    features: [
      { type: 'structure', name: 'Treehouse', position: { x: 0, y: 220 }, effect: 'Home runs through treehouse opening earn triple points' },
      { type: 'obstacle', name: 'Rope Ladder', position: { x: -100, y: 190 }, effect: 'Balls caught in ladder are automatic outs' },
      { type: 'bonus', name: 'Shade Zone', position: { x: 0, y: 100 }, effect: 'Balls in shade are harder to see' },
    ],
  ...
  },
  {
    ...
    features: [
      { type: 'hazard', name: 'Bayou Water', position: { x: 200, y: 150 }, effect: 'Balls in water are home runs (but lost balls!)' },
      { type: 'obstacle', name: 'Dock', position: { x: 180, y: 170 }, effect: 'Can catch balls off the dock for spectacular plays' },
      { type: 'weather', name: 'Humidity', position: { x: 0, y: 0 }, effect: 'Heavy air makes balls drop faster' },
    ],
  ...
  },
  {
    ...
    features: [
      { type: 'obstacle', name: 'Crane', position: { x: 0, y: 230 }, effect: 'Home runs that hit the crane bucket score 5x points!' },
      { type: 'structure', name: 'Concrete Mixer', position: { x: -160, y: 160 }, effect: 'Balls bounce unpredictably off construction equipment' },
      { type: 'bonus', name: 'Hard Hat Zone', position: { x: 120, y: 140 }, effect: 'Doubles scored here earn safety bonus points' },
    ],
...
async function handleSaveGame(request: Request, env: Env): Promise<Response> {
  try {
    ...
    if (!validation.success) {
      return errorResponse(`Validation error: ${validation.error.errors.map(e => e.message).join(', ')}`, 400);
    }
...
async function handleLoadGame(userId: string, env: Env): Promise<Response> {
  try {
    ...
    // Cache for 5 minutes
    await env.CACHE.put(`game:progress:${userId}`, JSON.stringify(gameData), { expirationTtl: 300 });
...
async function handleMatchResult(request: Request, env: Env): Promise<Response> {
  try {
    ...
    await env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, datetime('now'))
    `).bind(matchId, userId, opponentType, userScore, opponentScore, stadium).run();
      ...
      if (currentProgress) {
        await env.DB.prepare(`
          ...
          ON CONFLICT(user_id, category) DO UPDATE SET
            score = excluded.score,
            updated_at = datetime('now')
        `).bind(userId, currentProgress.total_wins).run();
      }
    }
    ...
    if (matchStats?.homeRuns && matchStats.homeRuns > 0) {
      await env.DB.prepare(`
        ...
        ON CONFLICT(user_id, category) DO UPDATE SET
          score = score + ?,
          updated_at = datetime('now')
      `).bind(userId, userId, matchStats.homeRuns, matchStats.homeRuns).run();
    }
    ...
    if (newCharacterUnlocks.length > 0 || newStadiumUnlocks.length > 0) {
      ...
      await env.DB.prepare(`
        UPDATE progress SET
          ...
          unlocked_stadiums = ?
        WHERE user_id = ?
      `).bind(JSON.stringify(updatedCharacters), JSON.stringify(updatedStadiums), userId).run();
    }
