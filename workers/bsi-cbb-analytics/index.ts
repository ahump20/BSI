...
async function computeLeagueContext(db: D1Database): Promise<LeagueContext> {
  // Batting aggregates
  ...
  if (pa <= 0) return defaultLeagueContext();
...
async function computeParkFactors(db: D1Database): Promise<number> {
  // Group games by home team to compute home vs away run differentials.
  // This requires processed_games table populated by bsi-cbb-ingest.
  const now = new Date().toISOString();
  ...
  // For now, insert neutral park factors (1.0) for all teams.
  // Real park factor computation requires home/away game-level data
  // which will be populated as the season progresses.
  const upsert = db.prepare(`
  ...
  const batch = teams.map(t =>
    upsert.bind(
      t.team, t.team_id, null, null, SEASON,
      1.0, 0, 'Baseline — insufficient home/away data for park factor computation', now,
    )
  );
...
async function computeConferenceStrength(db: D1Database): Promise<number> {
  ...
  const batch = conferences.map(c => {
    ...
    // Without inter-conference records and RPI data, use approximations
    const approxWinPct = 0.500; // Placeholder — needs game-level data
    const approxRPI = 0.500;    // Placeholder
    const strength = calculateConferenceStrength(approxWinPct, approxRPI, c.avg_woba, avgERA);
    const isPower = strength >= 65 ? 1 : 0;

    return upsert.bind(
      c.conference, SEASON, r1(strength), r2(c.avg_ops + avgERA),
      r2(avgERA), r3(c.avg_ops), r3(c.avg_woba),
      approxWinPct, approxRPI, isPower, now,
    );
  });
...
async function runAnalytics(env: Env, includeWeekly: boolean) {
  ...
  try {
    ...
    const league = await computeLeagueContext(env.DB);
    await env.KV.put(
