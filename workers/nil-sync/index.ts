...
async function recordSyncSuccess(
  ...
  const keyMap = {
    valuations: {
      success: KV_KEYS.NIL_VALUATIONS_LAST_SYNC,
      failures: KV_KEYS.NIL_VALUATIONS_FAILURES,
    },
    deals: { success: KV_KEYS.NIL_DEALS_LAST_SYNC, failures: KV_KEYS.NIL_DEALS_FAILURES },
    'player-stats': {
      success: KV_KEYS.PLAYER_STATS_LAST_SYNC,
      failures: KV_KEYS.PLAYER_STATS_FAILURES,
    },
  };
...
async function recordSyncFailure(
  ...
  const keyMap = {
    valuations: {
      error: KV_KEYS.NIL_VALUATIONS_LAST_ERROR,
      failures: KV_KEYS.NIL_VALUATIONS_FAILURES,
    },
    deals: { error: KV_KEYS.NIL_DEALS_LAST_ERROR, failures: KV_KEYS.NIL_DEALS_FAILURES },
    'player-stats': {
      error: KV_KEYS.PLAYER_STATS_LAST_ERROR,
      failures: KV_KEYS.PLAYER_STATS_FAILURES,
    },
  };
  const keys = keyMap[type];
...
async function getSyncStatus(
  ...
  const keyMap = {
    ...
    deals: {
      success: KV_KEYS.NIL_DEALS_LAST_SYNC,
      error: KV_KEYS.NIL_DEALS_LAST_ERROR,
      failures: KV_KEYS.NIL_DEALS_FAILURES,
    },
    'player-stats': {
      success: KV_KEYS.PLAYER_STATS_LAST_SYNC,
      error: KV_KEYS.PLAYER_STATS_LAST_ERROR,
...
async function syncNILValuations(env: Env): Promise<SyncResult> {
  ...
  try {
    ...
    const response = await fetch(
      'https://nil-athletes-api.p.rapidapi.com/athletes?sport=baseball&level=college',
      {
        headers: {
    ...
    for (const athlete of athletes) {
      ...
      const existing = await env.BSI_DB.prepare('SELECT id FROM nil_valuations WHERE player_id = ?')
        .bind(playerMatch.id)
        .first();
...
async function syncNILDeals(env: Env): Promise<SyncResult> {
  ...
  try {
    ...
    const response = await fetch(
      'https://nil-athletes-api.p.rapidapi.com/deals?sport=baseball&level=college&limit=100',
      {
        headers: {
          'X-RapidAPI-Key': env.NIL_RAPIDAPI_KEY,
          'X-RapidAPI-Host': NIL_ATHLETES_API_HOST,
    ...
    for (const deal of deals) {
      ...
      const playerMatch = await env.BSI_DB.prepare(
        ...
        .bind('%' + deal.athleteName + '%')
        .first<{ id: string; team_id: string }>();

      if (!playerMatch) continue;

      // Check if deal exists (by external ID)
      const existing = await env.BSI_DB.prepare('SELECT id FROM nil_deals WHERE id = ?')
        .bind(deal.id)
        .first();
...
async function handleHealth(env: Env): Promise<Response> {
  ...
  const httpStatus =
    overallStatus === 'healthy' ? 200 : overallStatus === 'unconfigured' ? 503 : 200;
...
async function handleStatus(env: Env): Promise<Response> {
  const [valStatus, dealStatus, valCount, dealCount] = await Promise.all([
    getSyncStatus(env, 'valuations'),
    getSyncStatus(env, 'deals'),
    env.BSI_DB.prepare('SELECT COUNT(*) as count FROM nil_valuations')
      .first<{ count: number }>()
      .catch(() => ({ count: 0 })),
    env.BSI_DB.prepare('SELECT COUNT(*) as count FROM nil_deals')
      .first<{ count: number }>()
      .catch(() => ({ count: 0 })),
  ]);
...
async function handleSyncAll(env: Env): Promise<Response> {
  ...
  const [valResult, dealResult] = await Promise.all([syncNILValuations(env), syncNILDeals(env)]);
...
async function handleGetValuations(env: Env, url: URL): Promise<Response> {
  ...
  try {
    ...
    return jsonResponse(
      { success: false, error: 'NIL tables not found. Run migration 006_nil_tables.sql first.' },
      500
    );
  }
}
    ...
    const results = {
      valuations:
        valResult.status === 'fulfilled'
          ? valResult.value
          : { success: false, error: String(valResult.reason) },
      deals:
        dealResult.status === 'fulfilled'
          ? dealResult.value
          : { success: false, error: String(dealResult.reason) },
    };

    const successCount = [valResult, dealResult].filter(
