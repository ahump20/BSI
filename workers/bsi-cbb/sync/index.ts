    ...
    if (body.valuations && body.valuations.length > 0) {
      for (const valuation of body.valuations) {
        try {
          ...
          errors.push(
            'Failed to insert valuation: ' + (e instanceof Error ? e.message : 'unknown')
          );
        }
      }
    }
...
async function syncHighlightlyTeams(
  env: CbbEnv,
  provider: HighlightlyProvider
): Promise<SyncResult> {
  const startTime = Date.now();
  let recordsFetched = 0;
  let recordsInserted = 0;
...
async function syncHighlightlyMatches(
  env: CbbEnv,
  provider: HighlightlyProvider
): Promise<SyncResult> {
  const startTime = Date.now();
  ...
  try {
    ...
    for (const match of matches) {
      try {
        ...
        const scores = parseScoreFromState(
          match.state?.score as Record<string, unknown> | undefined
        );
        if (existingId) {
          // Update existing
          await env.BSI_DB.prepare(
          ...
          const homeTeamId = await getEntityIdFromSource(
            env.BSI_DB,
            'team',
            'highlightly',
            match.homeTeam.id
          );
          const awayTeamId = await getEntityIdFromSource(
            env.BSI_DB,
            'team',
            'highlightly',
            match.awayTeam.id
...
function parseScoreFromState(score: Record<string, unknown> | undefined): {
  home: number | null;
  away: number | null;
} {
