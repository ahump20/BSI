...
export async function getTeamById(db: D1Database, id: string): Promise<TeamRow | null> {
  return db.prepare('SELECT * FROM college_baseball_teams WHERE id = ?').bind(id).first<TeamRow>();
}
...
export async function getTeamByHighlightlyId(
  db: D1Database,
...
export async function getPlayersByTeam(db: D1Database, teamId: string): Promise<PlayerRow[]> {
  const results = await db
    .prepare(
      'SELECT * FROM college_baseball_players WHERE team_id = ? AND is_active = 1 ORDER BY jersey_number'
    )
    .bind(teamId)
    .all<PlayerRow>();
  return results.results ?? [];
...
export async function searchPlayers(
  ...
  const results = await db
    .prepare('SELECT * FROM college_baseball_players WHERE name LIKE ? ORDER BY name LIMIT ?')
    .bind('%' + query + '%', limit)
    .all<PlayerRow>();
  return results.results ?? [];
}
...
export async function getGameById(db: D1Database, id: string): Promise<GameRow | null> {
  return db.prepare('SELECT * FROM college_baseball_games WHERE id = ?').bind(id).first<GameRow>();
}
...
export async function getStandings(
  ...
  query +=
    ' ORDER BY (r.conference_wins * 1.0 / NULLIF(r.conference_wins + r.conference_losses, 0)) DESC, r.overall_wins DESC';

  const results = await db
    .prepare(query)
    .bind(...params)
    .all();
  return (results.results ?? []) as unknown as (StandingsRow & {
    team_name: string;
...
export async function getPlayersForComparison(
  ...
  const results = await db
    ...
    .bind(...playerIds)
    .all();
  return (results.results ?? []) as unknown as (PlayerRow & {
    team_name: string;
    team_conference: string;
  })[];
}
...
export async function getPlayerStats(
  ...
  return db
    .prepare(
      "SELECT * FROM cbb_player_season_stats WHERE player_id = ? AND season = ? AND split_type = 'season'"
    )
...
export async function getPlayerCareerStats(
  ...
  const results = await db
    .prepare(
      "SELECT * FROM cbb_player_season_stats WHERE player_id = ? AND split_type = 'season' ORDER BY season DESC"
    )
    .bind(playerId)
    .all<PlayerStatsRow>();
  return results.results ?? [];
