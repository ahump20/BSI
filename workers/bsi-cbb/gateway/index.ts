...
app.get("/cbb/teams/:id/roster", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "Failed to fetch roster", message: error instanceof Error ? error.message : "Unknown error" }, 500);
  }
});
...
app.get("/cbb/d1baseball/mappings", async (c) => {
  ...
  if (conference) {
    ...
    const confTeams: Record<string, string[]> = {
      "big12": ["texas", "texas-tech", "tcu", "baylor", "oklahoma", "oklahoma-state", "kansas-state", "west-virginia", "ucf", "byu", "cincinnati", "houston", "kansas", "arizona", "arizona-state", "colorado", "utah"],
      "sec": ["texas-am", "lsu", "arkansas", "ole-miss", "mississippi-state", "tennessee", "vanderbilt", "florida", "georgia", "south-carolina", "auburn", "alabama", "kentucky", "missouri"],
      "acc": ["clemson", "florida-state", "miami-fl", "north-carolina", "nc-state", "duke", "wake-forest", "virginia", "virginia-tech", "louisville", "notre-dame", "pittsburgh", "georgia-tech"],
    };
...
app.post("/cbb/d1baseball/sync-conference/:conference", async (c) => {
  ...
  const confTeams: Record<string, string[]> = {
    ...
    "sec": ["texas-am", "lsu", "arkansas", "ole-miss", "mississippi-state", "tennessee", "vanderbilt", "florida", "georgia", "south-carolina", "auburn", "alabama", "kentucky", "missouri"],
    "acc": ["clemson", "florida-state", "miami-fl", "north-carolina", "nc-state", "duke", "wake-forest", "virginia", "virginia-tech", "louisville", "notre-dame", "pittsburgh", "georgia-tech"],
    "texas": ["texas", "texas-tech", "tcu", "baylor", "houston", "rice", "dallas-baptist", "sam-houston", "utsa", "texas-state", "ut-arlington", "ut-rio-grande-valley", "texas-am"],
  };
  ...
  for (const teamSlug of teamsToProcess) {
    ...
    try {
      ...
      if (roster) {
        const syncResult = await scraper.syncRosterToDatabase(env.BSI_DB, mapping.bsiTeamId, roster);
  ...
  return c.json({
    ...
    note: teamsToProcess.length < teamSlugs.length
...
app.get("/cbb/admin/cache-stats", async (c) => {
  ...
  return c.json({
    ...
    dataSources: {
      primary: { name: "Highlightly API", status: "active", sports: ["ncaa-baseball", "ncaa-football", "mlb", "nfl"] },
      fallback: { name: "ESPN API", status: "active", sports: ["college-football", "nfl", "mlb"], note: "ESPN college-baseball roster data unavailable" },
    },
  });
});
...
app.delete("/cbb/admin/cache/:prefix", async (c) => {
  ...
  try {
    ...
    return c.json({ success: true, prefix, keysDeleted: deleted, timestamp: new Date().toISOString() });
  } catch (e) {
    return c.json({ error: "Failed to clear cache", message: e instanceof Error ? e.message : "Unknown error" }, 500);
  }
});
...
app.get("/cbb/rankings", async (c) => {
  ...
  return c.json({
    success: true, poll: pollType ?? "all", season: season ?? new Date().getFullYear(),
    week: week ?? latestWeek,
    rankings: rankings.map((r) => ({
      rank: r.rank, previousRank: r.previous_rank, change: r.previous_rank ? r.previous_rank - r.rank : null,
      team: { id: r.team_id, name: r.team_name, logo: r.team_logo, conference: r.conference },
      points: r.points, firstPlaceVotes: r.first_place_votes, pollType: r.source, record: r.record,
    })), timestamp: new Date().toISOString(),
  });
});

app.get("/cbb/rankings/history/:teamId", async (c) => {
  const env = c.env; const teamId = c.req.param("teamId");
  const pollType = c.req.query("poll") ?? undefined;
  const season = c.req.query("season") ? parseInt(c.req.query("season")!) : undefined;
  const team = await getTeamById(env.BSI_DB, teamId);
  ...
  return c.json({
    success: true,
    team: { id: team.id, name: team.name, conference: team.conference },
    season: season ?? new Date().getFullYear(), poll: pollType ?? "all",
    history: history.map((r) => ({ week: r.week, rank: r.rank, previousRank: r.previous_rank,
      points: r.points, pollType: r.source, record: r.record })),
    timestamp: new Date().toISOString(),
  });
});
...
app.get("/cbb/players/:id/stats", async (c) => {
  const env = c.env; const playerId = c.req.param("id");
  const season = c.req.query("season") ? parseInt(c.req.query("season")!) : undefined;
  const player = await getPlayerById(env.BSI_DB, playerId);
  ...
  return c.json({
    ...
    currentSeason: stats, career: careerStats, timestamp: new Date().toISOString(),
  });
});

app.get("/cbb/teams/:id/leaders", async (c) => {
  const env = c.env; const teamId = c.req.param("id");
  const season = c.req.query("season") ? parseInt(c.req.query("season")!) : undefined;
  ...
  return c.json({
    success: true, team: { id: team.id, name: team.name, conference: team.conference },
    season: season ?? new Date().getFullYear(),
    batting: { average: battingAvg, homeRuns, rbi, stolenBases },
    pitching: { era, wins, strikeouts }, timestamp: new Date().toISOString(),
  });
});
