...
const leagueParams = {
  nfl: {
    ...
    optimalRanges: {
      ...
      processor: { min: 0.15, max: 0.25, peak: 0.20 }, // QB, MLB, slot WR
      connector: { min: 0.15, max: 0.25, peak: 0.20 }, // C, TE, MLB
      executor: { min: 0.20, max: 0.30, peak: 0.25 }, // OL, RB, DL
      catalyst: { min: 0.10, max: 0.20, peak: 0.15 }, // WR1, EDGE
    ...
    conflicts: [
      { pair: ['catalyst', 'catalyst'], threshold: 0.22, penalty: 0.10, reason: 'Variance overload' },
      { pair: ['regulator', 'regulator'], threshold: 0.15, penalty: 0.12, reason: 'Ball-dominant conflict' },
    ],
  ...
  mlb: {
    ...
    starterCount: 9,
    optimalRanges: {
      stabilizer: { min: 0.20, max: 0.30, peak: 0.25 }, // SP, C critical
      processor: { min: 0.15, max: 0.25, peak: 0.18 }, // SS, C, SP
      connector: { min: 0.18, max: 0.28, peak: 0.22 }, // C, SS, CF
      executor: { min: 0.15, max: 0.25, peak: 0.20 }, // 2B, 3B, relievers
      catalyst: { min: 0.12, max: 0.22, peak: 0.17 }, // CF, corner OF, power hitters
      regulator: { min: 0.08, max: 0.15, peak: 0.10 }, // Closer, DH
    },
    synergies: [
      { pair: ['stabilizer', 'connector'], bonus: 0.10, reason: 'Battery (C-SP) synergy' },
    ...
    conflicts: [
      { pair: ['catalyst', 'catalyst'], threshold: 0.25, penalty: 0.08, reason: 'K-rate vulnerability' },
      { pair: ['regulator', 'regulator'], threshold: 0.18, penalty: 0.10, reason: 'Clubhouse ego' },
    ],
    criticalPositions: [
  ...
  nba: {
    ...
    starterCount: 5,
    optimalRanges: {
      stabilizer: { min: 0.15, max: 0.25, peak: 0.20 }, // C, veteran presence
      processor: { min: 0.20, max: 0.30, peak: 0.25 }, // PG critical, all positions benefit
      connector: { min: 0.15, max: 0.25, peak: 0.20 }, // PG, glue guys
      executor: { min: 0.15, max: 0.25, peak: 0.18 }, // PF, defensive specialists
      catalyst: { min: 0.15, max: 0.25, peak: 0.20 }, // Wings, shot creators
    ...
    conflicts: [
      { pair: ['catalyst', 'catalyst'], threshold: 0.28, penalty: 0.08, reason: 'Shot distribution' },
      { pair: ['regulator', 'regulator'], threshold: 0.20, penalty: 0.15, reason: 'One ball problem' },
    ],
    criticalPositions: [
...
const scoreComposition = (composition: Composition, params: LeagueParams): number => {
  ...
  params.synergies.forEach(syn => {
    ...
    if (combined >= 0.35 && combined <= 0.50) {
      score += syn.bonus * 100;
    }
  });
...
const StatCard = ({ label, value, subtext, color }: StatCardProps) => (
  <div className="rounded-lg p-3" style={{ backgroundColor: tokens.colors.charcoal }}>
    ...
    {subtext && <div className="text-xs mt-1" style={{ color: tokens.colors.muted }}>{subtext}</div>}
  </div>
);
  ...
  return (
    <div className="flex items-end gap-0.5" style={{ height: maxHeight }}>
      {distribution.map((count, idx) => (
        <div key={idx} className="flex-1 rounded-t transition-all" 
          style={{ 
            height: max > 0 ? (count / max) * maxHeight : 0,
            backgroundColor: idx >= 16 ? tokens.colors.success : idx >= 12 ? tokens.colors.warning : tokens.colors.slate
          }}
    ...
    <div className="min-h-screen p-4" style={{ backgroundColor: tokens.colors.midnight, fontFamily: tokens.fonts.body }}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold mb-2" style={{ fontFamily: tokens.fonts.display, color: tokens.colors.bone }}>
          ...
          <p className="text-sm" style={{ color: tokens.colors.muted }}>
            Generates {iterations.toLocaleString()} random team compositions and identifies optimal archetype distributions based on research-derived scoring parameters.
          </p>
        </div>

        {/* Pro badge for subscribers */}
        {isPro && (
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full w-fit mb-4" style={{ backgroundColor: tokens.colors.burntOrange + '20', border: `1px solid ${tokens.colors.burntOrange}40` }}>
        ...
        {!isPro && (
          <div className="rounded-lg p-3 flex items-center justify-between mb-4" style={{ backgroundColor: tokens.colors.charcoal, border: `1px solid ${tokens.colors.slate}` }}>
            <span className="text-sm" style={{ color: tokens.colors.muted }}>
              <span style={{ color: tokens.colors.bone }}>Free preview:</span> 1,000 iterations, NFL only. Upgrade for 100K iterations and all leagues.
            </span>
        ...
        <div className="rounded-lg p-4 mb-4" style={{ backgroundColor: tokens.colors.charcoal }}>
          <div className="flex flex-wrap gap-4 items-end">
            <div>
              <label className="text-xs uppercase tracking-wider block mb-2" style={{ color: tokens.colors.subtle }}>League</label>
              <div className="flex gap-2">
                {Object.keys(leagueParams).map(l => {
                  const isLocked = !isPro && l !== 'nfl';
                  return (
                    <button key={l}
                      onClick={() => {
                        if (isLocked) return;
                      ...
                      style={{
                        backgroundColor: league === l ? tokens.colors.burntOrange : tokens.colors.midnight,
                        color: league === l ? '#fff' : isLocked ? tokens.colors.subtle : tokens.colors.muted,
            ...
            <div>
              <label className="text-xs uppercase tracking-wider block mb-2" style={{ color: tokens.colors.subtle }}>
                Iterations {!isPro && <span className="text-xs" style={{ color: tokens.colors.warning }}>(Pro: up to 100K)</span>}
              </label>
              <select value={iterations} onChange={e => setIterations(Number(e.target.value))}
                className="px-3 py-1.5 rounded text-sm"
                style={{ backgroundColor: tokens.colors.midnight, color: tokens.colors.bone, border: `1px solid ${tokens.colors.slate}` }}>
                {availableIterations.map(val => (
        ...
        {/* Methodology */}
        <div className="rounded-lg p-4 mb-4 text-sm" style={{ backgroundColor: tokens.colors.charcoal, borderLeft: `3px solid ${tokens.colors.burntOrange}` }}>
          <h3 className="font-semibold mb-2" style={{ color: tokens.colors.bone }}>Scoring Methodology</h3>
          <p style={{ color: tokens.colors.muted }} className="mb-2">
            Each composition is scored on: (1) proximity to optimal ranges derived from position requirements, 
            (2) synergy bonuses for complementary archetype pairs, (3) conflict penalties for overconcentration, 
            ...
            {Object.entries(params.optimalRanges).map(([arch, range]) => (
              <div key={arch} className="flex items-center gap-2 text-xs">
                ...
                <span style={{ color: tokens.colors.muted }}>
                  {archetypeConfig[arch].name}: {(range.min * 100).toFixed(0)}-{(range.max * 100).toFixed(0)}%
                </span>
              </div>
            ))}
          </div>
        ...
        {results && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <StatCard label="Best Score" value={results.bestScore.toFixed(1)} color={tokens.colors.success} />
              <StatCard label="Mean Score" value={results.mean.toFixed(1)} />
              <StatCard label="Std Dev" value={results.stdDev.toFixed(2)} />
              <StatCard label="Top 1%" value={`${results.topPct} teams`} subtext={`≥${(results.allResults[results.topPct - 1]?.score || 0).toFixed(1)} score`} />
            </div>

            {/* Distribution */}
            <div className="rounded-lg p-4" style={{ backgroundColor: tokens.colors.charcoal }}>
              <h3 className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: tokens.colors.subtle }}>
                Score Distribution
              </h3>
              <DistributionChart distribution={results.distribution} />
              <div className="flex justify-between mt-2 text-xs" style={{ color: tokens.colors.subtle }}>
                <span>0</span>
                <span>Score</span>
                <span>100</span>
              </div>
            ...
            <div className="rounded-lg p-4" style={{ backgroundColor: tokens.colors.charcoal }}>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: tokens.colors.success }} />
                <h3 className="text-xs font-semibold uppercase tracking-wider" style={{ color: tokens.colors.success }}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-4">
                {Object.entries(results.optimal)
                  .sort((a, b) => b[1] - a[1])
                  .map(([arch, pct]) => (
                    <div key={arch} className="flex items-center justify-between p-2 rounded"
                      ...
                      <span className="text-sm font-mono font-bold" style={{ color: archetypeConfig[arch].color }}>
                        {pct.toFixed(1)}%
                      </span>
                    </div>
                  ))}
            ...
            <div className="rounded-lg p-4" style={{ backgroundColor: tokens.colors.charcoal }}>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-xs font-semibold uppercase tracking-wider" style={{ color: tokens.colors.burntOrange }}>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {Object.entries(results.bestComposition)
                  .sort((a, b) => b[1] - a[1])
                  .map(([arch, pct]) => (
                    <span key={arch} className="text-xs px-2 py-1 rounded"
                      style={{ backgroundColor: archetypeConfig[arch].color + '30', color: archetypeConfig[arch].color }}>
            ...
            <div className="rounded-lg p-4" style={{ backgroundColor: tokens.colors.charcoal, borderLeft: `3px solid ${tokens.colors.processor}` }}>
              <h3 className="font-semibold mb-3" style={{ color: tokens.colors.bone }}>Research-Based Interpretation</h3>
              <div className="space-y-3 text-sm" style={{ color: tokens.colors.muted }}>
                {league === 'nfl' && (
                  <>
                    <p><strong style={{ color: tokens.colors.stabilizer }}>Stabilizer (23%):</strong> NFL-PAT validation shows Stress Tolerance as the #1 predictor for QB success. The optimal ~23% reflects QB, FS, and veteran leadership roles requiring emotional homeostasis under pressure.</p>
                    <p><strong style={{ color: tokens.colors.executor }}>Executor (25%):</strong> Offensive line (5 starters) demands conscientiousness and consistency. High Executor percentage reflects 10,000+ snap streaks like Joe Thomas and the thankless reliability required at OL/DL positions.</p>
                    <p><strong style={{ color: tokens.colors.processor }}>Processor (20%):</strong> AIQ Decision Making correlates with Passer Rating (Boone et al., 2025). Processor archetype essential at QB, MLB, and slot WR where cognitive processing speed determines success.</p>
                    <p><strong style={{ color: tokens.colors.catalyst }}>Catalyst (15%):</strong> Explosive plays shift momentum but increase variance. Research suggests limiting Catalyst exposure to WR1/EDGE roles where boom-or-bust profiles are tolerated.</p>
                  </>
                )}
                {league === 'mlb' && (
                  <>
                    <p><strong style={{ color: tokens.colors.stabilizer }}>Stabilizer (25%):</strong> Starting pitchers must maintain composure through 100+ pitches. The battery (C-SP) relationship requires Stabilizer presence. Madison Bumgarner's 2014 postseason exemplifies the archetype.</p>
                    <p><strong style={{ color: tokens.colors.connector }}>Connector (22%):</strong> Catcher leadership drives staff ERA differentials. Yadier Molina's teammate effects are the most documented Connector impact in baseball—measurable in runs prevented.</p>
                    <p><strong style={{ color: tokens.colors.catalyst }}>Catalyst (17%):</strong> AIQ Visual-Spatial Processing correlates with hitting (Bowman et al., 2021). Power hitters and range-maximizing center fielders benefit from Catalyst athleticism.</p>
                    <p><strong style={{ color: tokens.colors.regulator }}>Regulator (10%):</strong> Closers must seek pressure, not merely tolerate it. Failed closer conversions often stem from misdiagnosing Stabilizers (handle pressure) as Regulators (want pressure).</p>
                  </>
                )}
                {league === 'nba' && (
                  <>
                    <p><strong style={{ color: tokens.colors.processor }}>Processor (25%):</strong> AIQ predicts PER and eFG% beyond draft pick (Bowman et al., 2023). Point guard floor generalship and pick-and-roll decision-making require elite cognitive processing.</p>
                    <p><strong style={{ color: tokens.colors.catalyst }}>Catalyst (20%):</strong> Modern NBA emphasizes shot creation and spacing. Wing players need Catalyst burst for iso scoring while avoiding the "one ball problem" of too many ball-dominant players.</p>
                    <p><strong style={{ color: tokens.colors.stabilizer }}>Stabilizer (20%):</strong> Rim protection anchors defense psychologically. When centers like Rudy Gobert are rattled, help defense collapses. Stabilizer presence at C is non-negotiable.</p>
                    <p><strong style={{ color: tokens.colors.regulator }}>Regulator (12%):</strong> Clutch performance has modest year-over-year reliability (r = .12-.18), but designated closers like Kobe Bryant demonstrate the archetype's value in final possessions.</p>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="rounded-lg overflow-hidden" style={{ backgroundColor: tokens.colors.charcoal }}>
              <div className="p-3" style={{ borderBottom: `1px solid ${tokens.colors.slate}` }}>
                <h3 className="text-xs font-semibold uppercase tracking-wider" style={{ color: tokens.colors.subtle }}>Top 10 Compositions</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ backgroundColor: tokens.colors.midnight }}>
                      <th className="px-3 py-2 text-left" style={{ color: tokens.colors.subtle }}>#</th>
                      <th className="px-3 py-2 text-left" style={{ color: tokens.colors.subtle }}>Score</th>
                      <th className="px-3 py-2 text-left" style={{ color: tokens.colors.subtle }}>Composition</th>
                      {Object.keys(archetypeConfig).map(arch => (
                        <th key={arch} className="px-2 py-2 text-center" style={{ color: archetypeConfig[arch].color }}>
                          {archetypeConfig[arch].letter}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {results.allResults.slice(0, 10).map((r, idx) => (
                      <tr key={idx} style={{ borderTop: `1px solid ${tokens.colors.slate}` }}>
                        <td className="px-3 py-2" style={{ color: tokens.colors.muted }}>{idx + 1}</td>
                        <td className="px-3 py-2 font-mono" style={{ color: tokens.colors.success }}>{r.score.toFixed(1)}</td>
                        <td className="px-3 py-2 w-40"><CompositionBar composition={r.composition} height={8} /></td>
                        {Object.keys(archetypeConfig).map(arch => (
                          <td key={arch} className="px-2 py-2 text-center font-mono text-xs" style={{ color: tokens.colors.muted }}>
        ...
        {/* Footer */}
        <footer className="text-center py-6 mt-6" style={{ borderTop: `1px solid ${tokens.colors.slate}` }}>
          <p className="text-xs" style={{ color: tokens.colors.burntOrange }}>Born to Blaze the Path Less Beaten</p>
          <p className="text-xs mt-1" style={{ color: tokens.colors.subtle }}>
            © 2025 Blaze Sports Intel | Sources: NFL-PAT (Goldstein et al., 2013), AIQ (Bowman et al., 2020-2024), HEXACO (Ashton & Lee, 2007)
          </p>
        </footer>
      </div>
    </div>
