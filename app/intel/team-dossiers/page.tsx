          ...
          <Container>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div key={i} className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 animate-pulse">
                    ...
                    <div className="h-3 bg-white/[0.04] rounded w-1/2 mt-3" />
                  </div>
                ))}
              </div>
            )}
                  ...
                  <Link key={dossier.slug} href={`/intel/team-dossiers/${dossier.slug}`} className="block group">
                    <Card variant="default" padding="md" className="h-full hover:border-[#BF5700]/30 transition-all">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <h2 className="font-display text-sm font-bold text-white uppercase tracking-wide group-hover:text-[#BF5700] transition-colors">
                            {dossier.name}
                          </h2>
                          {dossier.mascot && <p className="text-white/30 text-xs">{dossier.mascot}</p>}
                        </div>
            ...
            <div className="mt-8 bg-white/[0.02] border border-dashed border-white/10 rounded-xl p-6 text-center">
              <p className="text-sm text-white/30">
                ...
                See{' '}
                <Link href="/college-baseball/editorial" className="text-[#BF5700] hover:text-[#FF6B35] transition-colors">
                  editorial previews
                </Link>{' '}
