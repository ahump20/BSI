...
const teamNames: Record<string, { name: string; abbreviation: string; division: string; league: string }> = {
...
export default function TeamDetailPage() {
  ...
  const teamInfo = teamNames[teamId] || { name: teamId, abbreviation: teamId.toUpperCase(), division: '', league: '' };
  ...
  const filterRoster = (players: Player[]) => {
    ...
    if (positionFilter === 'pitchers') return players.filter((p) => ['P', 'SP', 'RP', 'CL'].includes(p.position));
    if (positionFilter === 'catchers') return players.filter((p) => p.position === 'C');
    if (positionFilter === 'infielders') return players.filter((p) => ['1B', '2B', '3B', 'SS'].includes(p.position));
    if (positionFilter === 'outfielders') return players.filter((p) => ['LF', 'CF', 'RF', 'OF', 'DH'].includes(p.position));
    return players;
  };
      ...
      <>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredRoster.map((player) => (
            <Card key={player.id} variant="default" padding="md" className="hover:border-burnt-orange transition-all">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {player.stats && (
                    <div className="mt-2 text-xs">
                      {player.stats.avg && (
                        <span className="text-text-secondary font-mono">.{player.stats.avg.replace('0.', '')} AVG</span>
                      )}
                      {player.stats.era && (
                        <span className="text-text-secondary font-mono">{player.stats.era} ERA</span>
                      )}
                    </div>
                  )}
                </div>
        ...
        <Card variant="default" padding="lg">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <svg viewBox="0 0 24 24" className="w-5 h-5 text-burnt-orange" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              {positions.map((pos) => {
                ...
                return (
                  <div key={pos.position} className="bg-graphite rounded-lg p-4">
                    <p className="text-xs text-text-tertiary uppercase tracking-wide mb-2">{pos.label}</p>
                    {players.length > 0 ? (
                      <div className="space-y-2">
                        {players.slice(0, 2).map((player, idx) => (
                          <div key={player.id} className={`flex items-center gap-2 ${idx === 0 ? 'text-white' : 'text-text-secondary'}`}>
                            <span className="text-xs text-burnt-orange font-mono w-6">{player.number}</span>
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {pitchingRoles.map((role) => {
                const pitchers = team?.roster?.filter((p) =>
                  role.role === 'SP' ? p.position === 'SP' || p.position === 'P' :
                  role.role === 'CL' ? p.position === 'CL' :
                  p.position === 'RP'
                ) || [];
                return (
                  <div key={role.role}>
                    ...
                    {pitchers.length > 0 ? (
                      <div className="space-y-2">
                        {pitchers.slice(0, role.role === 'SP' ? 5 : role.role === 'CL' ? 1 : 6).map((player) => (
                          <div key={player.id} className="flex items-center justify-between bg-graphite rounded-lg px-3 py-2">
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-burnt-orange font-mono w-6">{player.number}</span>
                              <span className="text-sm text-text-secondary">{player.name}</span>
                            </div>
                            {player.stats?.era && (
                              <span className="text-xs text-text-tertiary font-mono">{player.stats.era}</span>
                            )}
                          </div>
                        ))}
                      </div>
        ...
        <Section padding="sm" className="border-b border-border-subtle">
          <Container>
            <nav className="flex items-center gap-2 text-sm">
              ...
              <Link href="/mlb/teams" className="text-text-tertiary hover:text-burnt-orange transition-colors">
                Teams
              </Link>
              <span className="text-text-tertiary">/</span>
