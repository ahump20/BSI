...
export default function MLBGameDetailPage() {
        ...
        <table className="w-full min-w-[600px] text-sm">
          <thead>
            <tr className="border-b border-border-subtle">
              ...
              <th className="text-center p-2 text-burnt-orange font-bold w-10 border-l border-border-subtle">R</th>
  ...
  const BoxScoreContent = () => {
    ...
    const BattingTable = ({ team, data, teamName }: { team: 'away' | 'home'; data: BattingLine[]; teamName: string }) => (
      <Card variant="default" padding="md" className="mb-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px] text-sm">
            ...
            <tbody>
              {data.map((line, idx) => (
                <tr key={idx} className={`border-b border-border-subtle ${line.h >= 2 ? 'bg-success/5' : ''}`}>
                  ...
                  <td className={`text-center p-2 font-mono ${line.h >= 2 ? 'text-success font-bold' : 'text-text-secondary'}`}>{line.h}</td>
    ...
    const PitchingTable = ({ team, data, teamName }: { team: 'away' | 'home'; data: PitchingLine[]; teamName: string }) => (
      <Card variant="default" padding="md" className="mb-6">
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <span className="w-8 h-8 bg-charcoal rounded-full flex items-center justify-center text-xs font-bold text-burnt-orange">
            {game?.teams[team].abbreviation}
                  ...
                  <td className="p-2 text-white">
                    ...
                    {line.decision && (
                      <span className={`ml-2 text-xs font-bold ${
                        line.decision === 'W' ? 'text-success' :
                        line.decision === 'L' ? 'text-error' :
                        line.decision === 'S' ? 'text-burnt-orange' :
                        'text-text-tertiary'
                      }`}>
                        ({line.decision})
                      </span>
    ...
    return (
      <div>
        <BattingTable team="away" data={game.boxscore.away.batting} teamName={game.teams.away.name} />
        <PitchingTable team="away" data={game.boxscore.away.pitching} teamName={game.teams.away.name} />
        <BattingTable team="home" data={game.boxscore.home.batting} teamName={game.teams.home.name} />
        <PitchingTable team="home" data={game.boxscore.home.pitching} teamName={game.teams.home.name} />
      </div>
    );
  };
              ...
              {plays.map((play) => (
                <div
                  ...
                  className={`p-3 rounded-lg ${
                    play.isScoring ? 'bg-burnt-orange/10 border-l-4 border-burnt-orange' : 'bg-graphite'
                  }`}
                >
                  ...
                  {play.isScoring && (
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <span className="text-xs text-text-tertiary">
                        Score: {game?.teams.away.abbreviation} {play.scoreAfter.away} - {game?.teams.home.abbreviation} {play.scoreAfter.home}
                      </span>
                    </div>
                  )}
                </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card variant="default" padding="md">
          ...
          <div className="space-y-4">
            {[
              { label: 'Batting Average', away: '.267', home: '.254' },
              { label: 'Slugging', away: '.432', home: '.398' },
              { label: 'OPS', away: '.764', home: '.721' },
              { label: 'Team ERA', away: '3.45', home: '3.89' },
            ].map((stat) => (
              <div key={stat.label} className="flex items-center justify-between">
                <span className="text-text-secondary font-mono">{stat.away}</span>
                <span className="text-text-tertiary text-sm">{stat.label}</span>
                <span className="text-text-secondary font-mono">{stat.home}</span>
              </div>
            ))}
          </div>
        </Card>
        <Card variant="default" padding="md">
          <div className="text-center py-8 text-text-tertiary">
            Advanced team stats coming soon
          </div>
        </Card>
      </div>
    );
        ...
        <Section padding="sm" className="border-b border-border-subtle">
          <Container>
            <nav className="flex items-center gap-2 text-sm">
              ...
              <Link href="/mlb/scores" className="text-text-tertiary hover:text-burnt-orange transition-colors">
                Scores
              </Link>
              <span className="text-text-tertiary">/</span>
              ...
              <Container>
                <ScrollReveal>
                  <div className="flex items-center gap-3 mb-4">
                    <Badge variant="secondary">{new Date(game.date).toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}</Badge>
                  ...
                  <div className="flex items-center justify-center gap-8 md:gap-16 py-6">
                    ...
                    <div className="text-center">
                      ...
                      <p className={`text-4xl font-bold font-mono mt-2 ${
                        game.status.isFinal && game.teams.away.isWinner ? 'text-white' : 'text-text-secondary'
                      }`}>
                    ...
                    <div className="text-center">
                      {game.status.isLive ? (
                        ...
                        <span className="text-burnt-orange font-semibold">{game.status.detailedState}</span>
                      )}
                    ...
                    <div className="text-center">
                      ...
                      <p className={`text-4xl font-bold font-mono mt-2 ${
                        game.status.isFinal && game.teams.home.isWinner ? 'text-white' : 'text-text-secondary'
                      }`}>
