...
export default function NFLScoresPage() {
  ...
  const fetchScores = useCallback(async (week: number) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/nfl/scores?week=${week}`);
      ...
      if (data.games) {
        setGames(data.games);
        setHasLiveGames(data.games.some((g: NFLGame) => g.status === 'live'));
      } else if (data.data) {
        setGames(data.data);
        setHasLiveGames(data.data.some((g: NFLGame) => g.status === 'live'));
      } else {
        setGames([]);
      }
      if (data.meta) {
  ...
  useEffect(() => {
    if (hasLiveGames) {
      const interval = setInterval(() => fetchScores(selectedWeek), 30000);
              ...
              <div>
                ...
                <ScrollReveal direction="up" delay={150}>
                  <p className="text-text-secondary mt-2">
                    Week-by-week NFL scores and game results
                  </p>
                </ScrollReveal>
              </div>
            </div>
            ...
            {loading ? (
              ...
              <Card variant="default" padding="lg">
                <div className="text-center py-8">
                  ...
                  <p className="text-text-secondary">No games scheduled for Week {selectedWeek}</p>
