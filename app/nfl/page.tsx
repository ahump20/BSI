      ...
      <>
        ...
        <Section padding="lg" background="charcoal" borderTop>
          <Container>
            <ScrollReveal>
              <div className="mb-8">
                <span className="text-[var(--bsi-primary)] text-xs font-semibold uppercase tracking-widest">Statistical Leaders</span>
                <h2 className="font-display text-2xl md:text-3xl font-bold uppercase tracking-display mt-1">
                  Player <span className="text-[var(--bsi-primary)]">Leaders</span>
                </h2>
              </div>
            </ScrollReveal>
                      ...
                      <Card variant="default" padding="lg" className="h-full">
                        <CardHeader>
                          <CardTitle size="sm" className="flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-[var(--bsi-primary)] inline-block" />
                            {catInfo.label}
                          </CardTitle>
                        </CardHeader>
                        <CardContent>
                            ...
                            {category.leaders.slice(0, 5).map((player, idx) => (
                              ...
                              >
                                <span className={`w-7 h-7 flex items-center justify-center rounded-full text-xs font-bold shrink-0 ${
                                  idx === 0 ? 'bg-[var(--bsi-primary)] text-white' : 'bg-[var(--bsi-primary)]/15 text-[var(--bsi-primary)]'
                                }`}>
                                  {idx + 1}
                                </span>
                                {player.headshot && (
                                ...
                                <div className="min-w-0 flex-1">
                                  <p className="text-sm font-semibold text-[var(--bsi-bone)] truncate">{player.name}</p>
                                  <p className="text-xs text-[rgba(196,184,165,0.5)]">{player.team}</p>
                                </div>
                                <div className="text-right shrink-0">
                                  <p className={`text-sm font-bold ${idx === 0 ? 'text-[var(--bsi-primary)]' : 'text-[var(--bsi-bone)]'}`}>
                                    {player.value}
                                  </p>
                                  <p className="text-[10px] text-[rgba(196,184,165,0.5)] uppercase">{catInfo.unit}</p>
                                </div>
                              </div>
                            ))}
                          </div>
                          <div className="mt-4 pt-3 border-t border-[var(--border-vintage)]">
                            <Link href="/nfl/players" className="text-[var(--bsi-primary)] text-xs font-semibold hover:text-[var(--bsi-primary)] transition-colors inline-flex items-center gap-1">
                              Full Leaderboard
        ...
        <Section padding="lg" background="midnight" borderTop>
          <Container>
            <ScrollReveal>
              <div className="mb-8">
                <span className="text-[var(--bsi-primary)] text-xs font-semibold uppercase tracking-widest">Intelligence</span>
                <h2 className="font-display text-2xl md:text-3xl font-bold uppercase tracking-display mt-1">
                  League <span className="text-[var(--bsi-primary)]">Intel</span>
                </h2>
                  ...
                  <Link href={card.href} className="block group h-full">
                    <Card variant="hover" padding="lg" className="h-full relative overflow-hidden">
                      ...
                      <div className="flex items-start gap-4">
                        <div className="w-10 h-10 rounded-sm bg-[var(--bsi-primary)]/15 flex items-center justify-center shrink-0">
                          {card.icon}
                        </div>
                        <div className="min-w-0 flex-1">
                          <Badge variant="secondary" className="text-[10px] mb-2">{card.tag}</Badge>
                          <h3 className="text-lg font-semibold text-[var(--bsi-bone)] mb-2 group-hover:text-[var(--bsi-primary)] transition-colors">
                            {card.title}
                          </h3>
                          <p className="text-[rgba(196,184,165,0.5)] text-sm leading-relaxed">
                            {card.description}
                          </p>
                          <span className="text-[var(--bsi-primary)] text-xs font-semibold mt-3 inline-flex items-center gap-1 group-hover:gap-2 transition-all">
                            Explore
                            <svg viewBox="0 0 24 24" className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
                          </span>
                        </div>
          ...
          <Container>
                ...
                <div className="space-y-6">
                  {[1, 2, 3].map((i) => (
                    <Card key={i} variant="default" padding="lg">
                      ...
                      <CardContent>
                        <table className="w-full"><thead><tr className="border-b-2 border-[var(--bsi-primary)]">
                            ...
                            <tbody>
                              {standingsByDivision[division].map((team, idx) => (
                                <tr key={team.teamName} className="border-b border-[var(--border-vintage)] hover:bg-[var(--surface-dugout)] transition-colors">
                                  ...
                                  <td className="p-3 text-[var(--bsi-dust)]">{team.ties || 0}</td>
                                  <td className="p-3 text-[var(--bsi-dust)]">{team.winPercentage.toFixed(3).replace('0.', '.')}</td>
                                  <td className="p-3 text-[var(--bsi-dust)]">{team.pointsFor || '-'}</td>
                                  <td className="p-3 text-[var(--bsi-dust)]">{team.pointsAgainst || '-'}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
            ...
            <TabPanel id="teams" activeTab={activeTab}>
              <div className="grid gap-6 md:grid-cols-2">
                {['AFC', 'NFC'].map((conf) => (
                  <div key={conf}>
                    <h3 className="text-xl font-display font-bold text-[var(--bsi-primary)] mb-4 flex items-center gap-2">
                      ...
                      {NFL_DIVISION_ORDER.filter((d) => d.startsWith(conf)).map((div) => {
                        ...
                        return (
                          <Card key={div} variant="default" padding="md">
                            <h4 className="text-sm font-semibold text-[rgba(196,184,165,0.5)] uppercase tracking-wider mb-3">{div}</h4>
                            <div className="space-y-2">
                                ...
                                return (
                                  <Link key={team.teamName} href={`/nfl/teams/${team.teamName.toLowerCase().replace(/\s+/g, '-')}`} className="block group">
                                    <div className={`flex items-center justify-between py-2 px-3 rounded-sm transition-colors ${
                                      isLeader ? 'bg-[var(--bsi-primary)]/10 border border-[var(--bsi-primary)]/20' : 'hover:bg-[var(--surface-dugout)]'
                                    }`}>
                                      <div className="flex items-center gap-3 min-w-0">
                                        <span className={`w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold ${
                                          isLeader ? 'bg-[var(--bsi-primary)] text-white' : 'bg-[var(--surface-dugout)] text-bsi-dust'
                                        }`}>
                                          {idx + 1}
                                        </span>
                                        <span className="text-sm font-semibold text-[var(--bsi-bone)] truncate group-hover:text-[var(--bsi-primary)] transition-colors">
                                          {team.teamName}
                                        </span>
                                      </div>
                                      <div className="flex items-center gap-3 shrink-0">
                                        <span className="text-xs text-[var(--bsi-dust)] font-mono">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                      {[
                        ...
                        <Link key={cat.label} href={cat.href} className="group block">
                          <div className="bg-[var(--surface-dugout)] rounded-sm p-4 border border-[var(--border-vintage)] hover:border-[var(--bsi-primary)] transition-colors h-full">
                            <h4 className="text-sm font-semibold text-[var(--bsi-bone)] group-hover:text-[var(--bsi-primary)] transition-colors">
                              {cat.label}
                            </h4>
                            <p className="text-[11px] text-[rgba(196,184,165,0.5)] mt-1 leading-snug">{cat.desc}</p>
                          </div>
                        </Link>
                      ))}
                    </div>
                        ...
                        {leaderCategories.slice(0, 3).map((cat) => {
                          ...
                          return (
                            <div key={cat.abbreviation} className="bg-[var(--surface-dugout)] rounded-sm p-4 border border-[var(--border-vintage)]">
                              <p className="text-[10px] text-[rgba(196,184,165,0.5)] uppercase tracking-wider mb-2">{catInfo.label} Leader</p>
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="text-sm font-semibold text-[var(--bsi-bone)]">{topPlayer.name}</p>
                                  <p className="text-xs text-[rgba(196,184,165,0.5)]">{topPlayer.team}</p>
                                </div>
                              </div>
                              <p className="text-2xl font-bold text-[var(--bsi-primary)] mt-2">{topPlayer.value}</p>
                            </div>
                          );
                        })}
                      </div>
                ...
                <Card variant="default" padding="lg" className="h-full">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-[rgba(196,184,165,0.5)] text-xs mt-0.5">What the advanced numbers actually mean</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    {NEXT_GEN_STATS_EXPLAINERS.map((item) => (
                      <div key={item.stat} className="border-b border-[var(--border-vintage)] last:border-0 pb-3 last:pb-0">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant="secondary" className="text-[10px] font-mono">{item.stat}</Badge>
                          <span className="text-sm font-semibold text-[var(--bsi-bone)]">{item.name}</span>
                        </div>
                        <p className="text-[rgba(196,184,165,0.5)] text-xs leading-relaxed">{item.description}</p>
                      </div>
                    ))}
                  </div>
                </Card>
