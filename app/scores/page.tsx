    ...
    async function fetchLiveCounts() {
      try {
        ...
        if (mlbRes.ok) {
          ...
          const gamesArray = Array.isArray(mlbData.games) ? mlbData.games : [];
          const mlbLive = gamesArray.filter((g: { status?: { isLive?: boolean } }) => g.status?.isLive).length;
          const mlbTotal = gamesArray.length;
