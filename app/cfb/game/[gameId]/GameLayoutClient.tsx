...
export function useGameData(): GameContextValue {
  ...
  if (!context) {
    throw new Error('useGameData must be used within CFBGameLayoutClient');
  }
  return context;
}
...
export default function CFBGameLayoutClient({ children }: GameLayoutClientProps) {
  const params = useParams();
  const pathname = usePathname();
  const gameId = params?.gameId as string;

  const [game, setGame] = useState<CFBGameData | null>(null);
          ...
          <>
            ...
            <Section padding="lg" background="charcoal">
              <Container>
                ...
                <CitationFooter
                  source={meta?.dataSource || 'ESPN College Football'}
