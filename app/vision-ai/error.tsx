...
export default function VisionAIError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error('[BSI:vision-ai] Route error:', error);
  }, [error]);
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-[var(--surface-dugout)] border border-[rgba(140,98,57,0.5)] rounded-sm p-8 text-center">
        ...
        <h2 className="text-xl font-bold text-[var(--bsi-bone)] mb-2">Vision AI unavailable</h2>
        <p className="text-[rgba(196,184,165,0.35)] text-sm mb-6">
          {error.message || 'Unable to load Vision AI data. Please try again.'}
        </p>
        <button
          onClick={reset}
