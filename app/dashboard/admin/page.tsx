  ...
  return (
    <div className="pt-6">
      <Section padding="lg">
        <Container>
          <div className="mb-8">
            <h1 className="font-display text-3xl font-bold uppercase text-[var(--bsi-bone)]">API Health Dashboard</h1>
            <p className="text-[var(--bsi-dust)] text-sm mt-1">{health?.timestamp ? new Date(health.timestamp).toLocaleString() : ''}</p>
          </div>

          {loading && !health ? (
            <div className="flex items-center justify-center py-20"><div className="w-10 h-10 border-4 border-[var(--bsi-primary)]/30 border-t-[var(--bsi-primary)] rounded-full animate-spin" /></div>
          ) : health ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card padding="lg">
                <h3 className="text-xs text-[rgba(196,184,165,0.35)] uppercase tracking-wider mb-2">KV Namespace</h3>
              ...
              <Card padding="lg">
                <h3 className="text-xs text-[rgba(196,184,165,0.35)] uppercase tracking-wider mb-2">D1 Database</h3>
              ...
              <Card padding="lg">
                <h3 className="text-xs text-[rgba(196,184,165,0.35)] uppercase tracking-wider mb-2">Highlightly API</h3>
                <Badge variant={statusColor(health.checks.highlightly.status)}>{health.checks.highlightly.status}</Badge>
                {health.checks.highlightly.latency_ms != null && <p className="text-xs text-[var(--bsi-dust)] mt-2">{health.checks.highlightly.latency_ms}ms latency</p>}
                {health.checks.highlightly.rateLimitRemaining != null && <p className="text-xs text-[var(--bsi-dust)]">{health.checks.highlightly.rateLimitRemaining} requests remaining</p>}
              </Card>
              <Card padding="lg">
                <h3 className="text-xs text-[rgba(196,184,165,0.35)] uppercase tracking-wider mb-2">Recent Errors</h3>
                <span className={`text-2xl font-bold ${health.checks.recentErrors > 0 ? 'text-error' : 'text-success'}`}>{health.checks.recentErrors}</span>
                <p className="text-xs text-[rgba(196,184,165,0.35)] mt-1">Last 7 days</p>
              </Card>
            </div>
          ) : null}

          <button onClick={fetchHealth} disabled={loading} className="mt-6 px-4 py-2 bg-[var(--surface-dugout)] text-[var(--bsi-dust)] hover:text-[var(--bsi-bone)] rounded-sm text-sm transition-colors disabled:opacity-50">
