...
export function useGameData() {
  ...
  if (!context) {
    throw new Error('useGameData must be used within NBAGameLayout');
  }
  return context;
}
...
export default function GameLayoutClient({ children }: GameLayoutClientProps) {
  ...
  const fetchGame = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/nba/game/${gameId}`);
  ...
  // Auto-refresh for live games (15s)
  useEffect(() => {
  ...
  const tabs = [
    ...
    { id: 'play-by-play', label: 'Play-by-Play', href: `/nba/game/${gameId}/play-by-play` },
    { id: 'team-stats', label: 'Team Stats', href: `/nba/game/${gameId}/team-stats` },
    { id: 'recap', label: 'Recap', href: `/nba/game/${gameId}/recap` },
  ];
