        ...
        <Section padding="lg">
          <Container size="narrow">
            ...
            {(hasLiveHealth || loading) && (
              <section className="mb-16">
                ...
                {loading && !hasLiveHealth ? (
                  <div className="bg-surface-light border border-border-subtle rounded-sm p-5 animate-pulse">
                    <div className="space-y-3">
                      {[1, 2, 3].map((i) => (
                        <div key={i} className="h-10 bg-surface-light rounded-sm" />
                      ))}
                    </div>
                  </div>
                ) : hasLiveHealth ? (
                    ...
                    {Object.entries(health!.providers).map(([name, provider]) => (
                      <div key={name} className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm text-text-secondary font-medium capitalize">{name}</span>
                        </div>
              ...
              <div className="bg-surface-light border border-border-subtle rounded-sm p-5 sm:p-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-surface-light rounded-sm p-4">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-burnt-orange">KV Cache Hit</span>
                    <p className="text-2xl font-display text-text-primary mt-1">&lt;10ms</p>
                    <p className="text-xs text-text-muted mt-1">Pre-warmed by cron every 60s during in-season</p>
                  </div>
                  <div className="bg-surface-light rounded-sm p-4">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-burnt-orange">Cache Miss</span>
                    <p className="text-2xl font-display text-text-primary mt-1">200–800ms</p>
                    <p className="text-xs text-text-muted mt-1">Fetches from upstream provider, writes to KV, returns</p>
                  </div>
                </div>
                <p className="text-xs text-text-muted mt-4">
                  Health endpoint:{' '}
                  <code className="text-text-muted bg-surface-light px-1.5 py-0.5 rounded-sm font-mono">
                    /api/health
                  </code>{' '}
            ...
            <section className="mb-16">
              ...
              <div className="bg-surface-light border border-border-subtle rounded-sm p-5 sm:p-6">
                <p className="text-sm text-text-tertiary leading-relaxed mb-4">
                  Every BSI API response includes a <code className="text-text-secondary bg-surface-light px-1.5 py-0.5 rounded-sm text-xs font-mono">meta</code> object
                  that tells you exactly when the data was fetched and where it came from:
                </p>
                <pre className="bg-surface-light rounded-sm p-4 text-xs font-mono text-text-tertiary overflow-x-auto leading-relaxed">
{`{
  "data": { ... },
  "meta": {
    "source": "highlightly",
    ...
    "timezone": "America/Chicago"
                ...
                <div className="mt-4 space-y-2">
                  <div className="flex gap-3 text-sm">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-text-muted mt-0.5 shrink-0 w-20">source</span>
                    <span className="text-text-muted">Which provider served this data</span>
                  </div>
                  <div className="flex gap-3 text-sm">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-text-muted mt-0.5 shrink-0 w-20">fetched_at</span>
                    <span className="text-text-muted">ISO 8601 timestamp of when the Worker fetched from the provider</span>
                  </div>
                  <div className="flex gap-3 text-sm">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-text-muted mt-0.5 shrink-0 w-20">timezone</span>
                    <span className="text-text-muted">Always America/Chicago — BSI normalizes all timestamps to Central</span>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="mb-16">
              <div className="bg-surface-light border border-border-subtle rounded-sm p-6 sm:p-8">
                ...
                <div className="text-sm text-text-tertiary leading-relaxed space-y-3">
                  ...
                  <p>
                    Every API response carries a <code className="text-text-secondary bg-surface-light px-1.5 py-0.5 rounded-sm text-xs font-mono">meta</code> object
                    with <code className="text-text-secondary bg-surface-light px-1.5 py-0.5 rounded-sm text-xs font-mono">source</code>,{' '}
                    <code className="text-text-secondary bg-surface-light px-1.5 py-0.5 rounded-sm text-xs font-mono">fetched_at</code>, and{' '}
                    <code className="text-text-secondary bg-surface-light px-1.5 py-0.5 rounded-sm text-xs font-mono">timezone</code>.
            ...
            <div className="mt-12 flex flex-wrap gap-4 text-sm text-text-muted">
              ...
              <Link href="/data-sources" className="hover:text-text-secondary transition-colors">
