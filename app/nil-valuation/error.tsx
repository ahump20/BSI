...
export default function NILValuationError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error('[BSI:nil-valuation] Route error:', error);
  }, [error]);
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-background-secondary border border-border-strong rounded-sm p-8 text-center">
        ...
        <h2 className="text-xl font-bold text-text-primary mb-2">NIL data unavailable</h2>
        <p className="text-text-muted text-sm mb-6">
          {error.message || 'Unable to load NIL valuation data. This may be a temporary issue.'}
        </p>
        <button
          onClick={reset}
