      ...
      <Section className="pt-4 pb-0">
        <Container>
          <nav className="flex items-center gap-2 text-sm text-[rgba(196,184,165,0.35)]">
            ...
            <span>/</span>
            <span className="text-[var(--bsi-dust)]">Draft Leverage</span>
          </nav>
        </Container>
      </Section>
          ...
          <ScrollReveal>
            <div className="max-w-3xl mx-auto text-center">
              ...
              <h1 className="text-4xl md:text-5xl font-bold mb-4 font-display uppercase tracking-wide">
                <span className="text-[var(--bsi-primary)]">Draft</span> Leverage
              </h1>
        ...
        <Container>
          {!hasAccess ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-[rgba(196,184,165,0.35)]">Loading draft leverage data...</p>
            </div>
          ) : (
            <>
                  ...
                  <ScrollReveal key={q.key} delay={i * 80}>
                    <Card className={`text-center border-l-4 ${q.borderColor}`}>
                      <CardContent className="p-4">
                        ...
                        <div className="text-sm font-semibold text-[var(--bsi-bone)] mt-1">{q.title}</div>
                        <div className="text-xs text-[rgba(196,184,165,0.35)]">{q.subtitle}</div>
                      </CardContent>
                    </Card>
                  </ScrollReveal>
                ))}
                  ...
                  <ScrollReveal key={q.key} delay={qi * 100}>
                    <Card className={`border-l-4 ${q.borderColor}`}>
                      <CardContent className="p-6">
                        ...
                        <p className="text-sm text-[rgba(196,184,165,0.5)] mb-4">{q.description}</p>
                            ...
                            {(grouped[q.key] || []).slice(0, 8).map(p => (
                              <div key={p.player_id} className="flex items-center justify-between py-2 border-b border-border/30 last:border-0">
                                ...
                                <div className="text-right">
                                  <span className="text-sm font-bold text-[var(--bsi-primary)]">{formatValue(p.estimated_mid)}</span>
                                  <span className="text-xs text-[rgba(196,184,165,0.35)] ml-2">Perf: {p.performance_score}</span>
                                </div>
                              </div>
                            ))}
                            {(grouped[q.key] || []).length > 8 && (
                              <p className="text-xs text-[rgba(196,184,165,0.35)] text-center pt-2">
                                +{(grouped[q.key] || []).length - 8} more
                              </p>
                            )}
                          </div>
                        ) : (
                          <p className="text-sm text-[rgba(196,184,165,0.35)]">No players in this quadrant yet.</p>
                        )}
                      </CardContent>
                    </Card>
                  </ScrollReveal>
