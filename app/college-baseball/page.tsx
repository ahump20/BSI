...
// Conference list from canonical registry with dynamic team counts
const conferenceList = (() => {
  const confMap = new Map<string, number>();
  for (const team of Object.values(teamMetadata)) {
    const normalized = normalizeCollegeBaseballConference(team.conference);
    if (!normalized) continue;
    confMap.set(normalized, (confMap.get(normalized) || 0) + 1);
  }

  const entries = COLLEGE_BASEBALL_CONFERENCES.map((conference) => ({
    name: conference.id,
    teams: confMap.get(conference.id) || 0,
    href: `/college-baseball/standings?conference=${conference.slug}`,
    isPrimary: conference.isPrimary,
  })).sort((a, b) => Number(!a.isPrimary) - Number(!b.isPrimary) || a.name.localeCompare(b.name));
...
const scheduleConferences = [
  'All',
  ...conferenceList.filter((c) => c.name !== 'All Conferences').map((c) => c.name),
];
...
function CollegeBaseballPageInner() {
  ...
  const [apiSearchResults, setApiSearchResults] = useState<
    Array<{ name: string; href: string; category?: string }>
  >([]);
    ...
    return Object.entries(teamMetadata)
      .filter(
        ([slug, meta]) =>
          ...
          meta.conference?.toLowerCase().includes(q) ||
          slug.includes(q),
      )
      .slice(0, 8)
      .map(([slug, meta]) => ({
    ...
    searchTimerRef.current = setTimeout(async () => {
      try {
        const resp = await fetch(
          `/api/search?q=${encodeURIComponent(query)}&sport=college-baseball`,
  ...
  const previousRankings = useMemo(() => {
    ...
    return normed.teams.length
      ? normed.teams.map((t) => ({ rank: t.rank, team: t.team }))
      : undefined;
  ...
  const scheduleUrl =
    activeTab === 'schedule' ? `/api/college-baseball/schedule?date=${selectedDate}` : null;
  const {
    data: scheduleRaw,
  ...
  const hasLiveGames = useMemo(
    () => scheduleGames.some((g) => g.status === 'live'),
    [scheduleGames],
  );
  useEffect(() => {
  ...
  const { data: teamsRaw, loading: teamsLoading } = useSportData<{ teams?: TeamListItem[] }>(
    teamsUrl,
  );
  ...
  const dataSource =
    rankingsRaw?.meta?.dataSource ||
    standingsRaw?.meta?.dataSource ||
    scheduleRaw?.meta?.dataSource ||
    'ESPN';
  const lastUpdated =
    rankingsRaw?.meta?.lastUpdated ||
    standingsRaw?.meta?.lastUpdated ||
    scheduleRaw?.timestamp ||
    scheduleRaw?.meta?.lastUpdated ||
    '';
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      {
                        label: 'Swing Intel',
                        desc: 'AI-powered swing analysis from video',
                        href: '/swing',
                      ...
                      {
                        label: 'Editorial',
                        desc: 'Weekly recaps and previews',
                        href: '/college-baseball/editorial',
                        icon: 'M19 20H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v1m2 13a2 2 0 0 1-2-2V7m2 13a2 2 0 0 1 2-2V9a2 2 0 0 1-2-2h-2',
                      },
                      {
                        label: 'Compare',
                        desc: 'Head-to-head team matchups',
                    ...
                    <button
                      onClick={() =>
                        setSelectedDate(
                          getDateOffset(
                            Math.round(
                              (new Date(selectedDate).getTime() - new Date().getTime()) / 86400000,
                            ) - 3,
                          ),
                        )
                      }
                  ...
                  <div>
                    <div className="flex flex-col sm:flex-row gap-3 mb-6">
                      ...
                      >
                        ...
                        {conferenceList
                          .filter((c) => c.name !== 'All Conferences')
                          .map((c) => (
                    ...
                    {teamsLoading ? (
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                        {filteredTeams.map((team) => {
                          const meta = Object.entries(teamMetadata).find(
                            ([, m]) =>
                              m.shortName.toLowerCase() ===
                              (team.shortName || team.name).toLowerCase(),
                          )?.[1];
                          return (
