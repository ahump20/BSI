...
export default class ConferenceStrengthService {
  constructor(cache, logger, defaults = {}) {
    this.cache = cache instanceof CacheService || typeof cache?.get === 'function' ? cache : undefined;
    this.logger = logger instanceof LoggerService || logger?.logHttpRequest ? logger : logger;
    this.defaults = {
  ...
  async forecastStrength(snapshot, options = {}) {
    const resolvedOptions = {
      ...
      deterministic: typeof options.deterministic === 'boolean' ? options.deterministic : options.mode === 'basic',
    ...
    const samples = resolvedOptions.deterministic
  ...
  buildScenarios(samples, baseline) {
    if (samples.length === 1) {
  ...
  estimateWinProbability(snapshot, opponent) {
  ...
  computeRpi(snapshot, wins, losses) {
    ...
    const totalFutureGames = snapshot.futureOpponents.reduce(
      (acc, opponent) => acc + (opponent.games && opponent.games > 0 ? opponent.games : 1),
      0
    );
  ...
  computeSor(snapshot, wins, losses) {
    const totalGames = wins + losses;
    if (totalGames === 0) {
  ...
  estimateOpponentWinningPct(opponent) {
  ...
  buildCacheKey(snapshot, options) {
  ...
  async cacheResult(cacheKey, result, ttlSeconds) {
    if (!this.cache) {
      return;
    }
