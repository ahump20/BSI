/**
 * Blaze Sports Intel - Touch Gesture Library
 * PHASE 20-D: Mobile-First Touch Interactions
 *
 * Features:
...
class BlazeGestureHandler {
    ...
    init() {
        ...
        if (this.options.debug) {
            console.log('[BlazeGestures] Initialized with options:', this.options);
        }
    }
    ...
    handleTouchStart(e) {
        ...
        if (this.options.enablePinch && e.touches.length === 2) {
            this.isPinching = true;
            this.initialPinchDistance = this.getPinchDistance(e.touches);
            if (this.options.debug) {
                console.log('[BlazeGestures] Pinch started:', this.initialPinchDistance);
            }
        }
        ...
        // Handle double tap
        if (this.options.enableDoubleTap && e.touches.length === 1) {
            const now = Date.now();
    ...
    handleTouchMove(e) {
        ...
        if (this.isPinching && e.touches.length === 2) {
            ...
            if (Math.abs(scale - 1) > this.options.pinchThreshold) {
                if (scale > 1) {
                    this.trigger('pinchOut', { scale, delta });
                } else {
                    this.trigger('pinchIn', { scale, delta });
                }
        ...
        if (this.options.enablePullToRefresh && e.touches.length === 1) {
            const touch = e.touches[0];
            const deltaY = touch.clientY - this.touchStart.y;

            // Only trigger if pulling down from top of page
            if (window.scrollY === 0 && deltaY > 0) {
                if (deltaY > this.options.pullToRefreshThreshold && !this.pullToRefreshActive) {
                    this.pullToRefreshActive = true;
                    this.trigger('pullToRefresh', { distance: deltaY });
                }
    ...
    handleTouchEnd(e) {
        ...
        if (this.options.enableSwipe && e.changedTouches.length === 1) {
            ...
            if (this.options.debug) {
                console.log('[BlazeGestures] Swipe:', { deltaX, deltaY, velocity });
            }

            // Determine swipe direction
            if (Math.abs(deltaX) > Math.abs(deltaY)) {
                // Horizontal swipe
                if (Math.abs(deltaX) > this.options.swipeThreshold && velocity > this.options.swipeVelocity) {
                    if (deltaX > 0) {
                        this.trigger('swipeRight', { distance: deltaX, velocity });
                    } else {
                        this.trigger('swipeLeft', { distance: Math.abs(deltaX), velocity });
                    }
                }
            } else {
                // Vertical swipe
                if (Math.abs(deltaY) > this.options.swipeThreshold && velocity > this.options.swipeVelocity) {
                    if (deltaY > 0) {
                        this.trigger('swipeDown', { distance: deltaY, velocity });
                    } else {
                        this.trigger('swipeUp', { distance: Math.abs(deltaY), velocity });
                    }
                }
            }
        }
    ...
    on(event, callback) {
        if (this.handlers[event]) {
            ...
            console.warn('[BlazeGestures] Unknown event:', event);
        }
        return this;
    }
    ...
    trigger(event, data = {}) {
        if (this.handlers[event]) {
            if (this.options.debug) {
                console.log('[BlazeGestures] Trigger:', event, data);
            }
...
/**
 * SwipeNavigation - Navigate between sections with swipe gestures
 */
class SwipeNavigation {
    constructor(sections, options = {}) {
        this.sections = Array.isArray(sections) ? sections : Array.from(document.querySelectorAll(sections));
        this.currentIndex = 0;
        this.options = {
        ...
        this.gesture = new BlazeGestureHandler({
            enablePinch: false,
    ...
    init() {
        this.gesture.on('swipeLeft', () => this.next());
        this.gesture.on('swipeRight', () => this.previous());
    ...
    showSection(index) {
        this.sections.forEach((section, i) => {
            if (i === index) {
                ...
                setTimeout(() => {
                    section.style.transition = `opacity ${this.options.animationDuration}ms ease-in-out`;
        ...
        this.options.onNavigate(index, this.sections[index]);
    }
 ...
 */
class PullToRefresh {
    constructor(container, refreshCallback, options = {}) {
        this.container = typeof container === 'string' ? document.querySelector(container) : container;
        this.refreshCallback = refreshCallback;
        this.options = {
            threshold: options.threshold || 80,
            refreshText: options.refreshText || 'Pull to refresh',
            releaseText: options.releaseText || 'Release to refresh',
    ...
    init() {
        ...
        this.indicator.innerHTML = `
            <div class="pull-to-refresh-content">
                ...
                <span>${this.options.refreshText}</span>
            </div>
        `;
        ...
        document.addEventListener('touchmove', (e) => {
            if (!canPull || this.isRefreshing) return;

            const currentY = e.touches[0].clientY;
            this.pullDistance = Math.max(0, currentY - startY);

            if (this.pullDistance > 0) {
                const translateY = Math.min(this.pullDistance, this.options.threshold + 20);
                this.indicator.style.transform = `translateY(${translateY}px)`;

                const icon = this.indicator.querySelector('.fas');
...
/**
 * TouchOptimizedTable - Make tables scrollable and touch-friendly
 */
class TouchOptimizedTable {
    constructor(table, options = {}) {
        this.table = typeof table === 'string' ? document.querySelector(table) : table;
        this.options = {
            enableHorizontalScroll: options.enableHorizontalScroll !== false,
            highlightOnTouch: options.highlightOnTouch !== false,
            ...options
        };
    ...
    init() {
        ...
        wrapper.className = 'touch-table-wrapper';
        ...
        if (this.options.highlightOnTouch) {
            const rows = this.table.querySelectorAll('tbody tr');
            rows.forEach(row => {
                row.addEventListener('touchstart', () => {
                    row.style.backgroundColor = 'rgba(191, 87, 0, 0.1)';
                }, { passive: true });
...
if (typeof window !== 'undefined') {
    window.BlazeGestureHandler = BlazeGestureHandler;
    window.SwipeNavigation = SwipeNavigation;
    window.PullToRefresh = PullToRefresh;
    window.TouchOptimizedTable = TouchOptimizedTable;
}

console.log('[BlazeGestures] Touch gesture library loaded v1.0.0');
