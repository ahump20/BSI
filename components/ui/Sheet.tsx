      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className={`fixed z-[1050] overflow-y-auto bg-[var(--bsi-bg-secondary,#161620)] border-l border-white/10 ${positionClass} ${className}`}
          >
            {children}
          </motion.div>
        </>
...
export function SheetHeader({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`sticky top-0 z-10 px-6 py-4 border-b border-white/10 bg-[var(--bsi-bg-secondary,#161620)] ${className}`}>
      {children}
    </div>
  );
}
