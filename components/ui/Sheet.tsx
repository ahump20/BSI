      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className={`fixed z-[1050] overflow-y-auto bg-background-secondary border-l border-border ${positionClass} ${className}`}
          >
            {children}
          </motion.div>
        </>
...
export function SheetHeader({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`sticky top-0 z-10 px-6 py-4 border-b border-border bg-background-secondary ${className}`}>
      {children}
    </div>
  );
}
