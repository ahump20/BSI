...
export function SortableTh({ label, sortKey, indicator, onSort, className = '' }: SortableThProps) {
  return (
    <th
      scope="col"
      className={`text-left p-3 text-[var(--bsi-dust)] font-semibold text-xs uppercase tracking-wider cursor-pointer select-none hover:text-[var(--bsi-primary)] transition-colors ${className}`}
