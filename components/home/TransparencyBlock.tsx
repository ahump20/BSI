...
export function TransparencyBlock() {
  return (
    <div className="glass-default rounded-2xl p-6 sm:p-8 border border-white/[0.06] max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <div className="relative flex h-2.5 w-2.5">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75" />
          <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-green-500" />
        </div>
        <h2 className="text-lg font-display text-white uppercase tracking-wide">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {TRANSPARENCY_ITEMS.map((item) => (
          <div key={item.label} className="flex gap-3">
            <span className="text-[10px] font-bold uppercase tracking-wider text-[#BF5700] mt-0.5 shrink-0 w-16">
              {item.label}
            </span>
