...
export function IntelSignup({ sport, onSignup }: IntelSignupProps = {}) {
  ...
  if (state === 'success') {
    return (
      <div className="bg-surface-dugout border rounded-sm p-4 mb-4" style={{ borderColor: 'rgba(191, 87, 0, 0.2)' }}>
        <p className="text-sm text-bsi-primary font-medium">You&apos;re in.</p>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface-dugout border border-border-vintage rounded-sm p-4 mb-4">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="shrink-0 bg-gradient-to-r from-burnt-orange to-burnt-orange/80 hover:from-burnt-orange/90 hover:to-burnt-orange text-white px-4 py-2 rounded-sm text-sm font-semibold transition-all duration-300 disabled:opacity-50"
        >
      ...
      {state === 'error' && errorMsg && (
        <p className="text-xs text-error mt-2">{errorMsg}</p>
      )}
    </form>
  );
}
