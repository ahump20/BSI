    ...
    <article className="max-w-3xl">
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-success/5 border border-success/10 rounded-sm p-5">
            <h3 className="text-xs font-bold uppercase tracking-wider text-success/70 mb-3">
              How They Win
            </h3>
            <ul className="space-y-2">
              {dossier.howTheyWin.map((item, i) => (
                <li key={i} className="flex gap-2 text-sm text-text-muted leading-relaxed">
                  <span className="text-success/50 mt-1 shrink-0">+</span>
                  {item}
                </li>
              ))}
            </ul>
          </div>
          <div className="bg-error/5 border border-error/10 rounded-sm p-5">
            <h3 className="text-xs font-bold uppercase tracking-wider text-error/70 mb-3">
              How They Lose
            </h3>
            <ul className="space-y-2">
              {dossier.howTheyLose.map((item, i) => (
                <li key={i} className="flex gap-2 text-sm text-text-muted leading-relaxed">
                  <span className="text-error/50 mt-1 shrink-0">-</span>
                  {item}
                </li>
              ))}
            </ul>
