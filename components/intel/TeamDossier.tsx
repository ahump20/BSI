    ...
    <article className="max-w-3xl">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-white/30 text-sm">{dossier.date}</span>
          <span className="text-white/20 text-sm">{dossier.readTime}</span>
        </div>
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h1 className="font-display text-3xl md:text-4xl font-bold uppercase tracking-wide text-white">
              {dossier.name}
            </h1>
            <p className="text-white/40 text-lg">{dossier.mascot}</p>
          </div>
          <div className="text-right shrink-0">
            <TierBadge tier={dossier.tier} />
            <p className="text-white/30 font-mono text-sm mt-2">{dossier.record}</p>
          </div>
        </div>
        ...
        <section className="mb-10">
          ...
          <div className="space-y-3">
            {dossier.keyPlayers.map((player) => (
              <div
                key={player.name}
                className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-4 flex items-start justify-between gap-4"
              >
                <div>
                ...
                <span className="text-xs font-mono text-white/40 shrink-0">{player.statLine}</span>
              </div>
            ))}
          </div>
        </section>
      ...
      <section className="mb-10">
        ...
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-5">
          <p className="text-sm text-white/50 leading-relaxed">{dossier.scheduleDifficulty}</p>
        </div>
      </section>
      ...
      <section className="mb-10">
        ...
        <div className="bg-[#BF5700]/5 border border-[#BF5700]/15 rounded-xl p-5">
          <p className="text-sm text-white/50 leading-relaxed">{dossier.outlook}</p>
        </div>
      </section>
    </article>
  );
