...
interface MLBGamesListProps {
  date?: string;
  limit?: number;
}

export function MLBGamesList({ date, limit = 6 }: MLBGamesListProps) {
  ...
  useEffect(() => {
    async function fetchGames() {
      try {
        ...
        const response = await fetch(`/api/mlb/games?${params}`);
      ...
      <Card className="text-center py-12">
        <CardContent>
          ...
          <p className="text-sm text-text-tertiary mt-2">
            MLB AI previews and recaps will be available during the season.
          </p>
        </CardContent>
      </Card>
    );
      ...
      <Card className="text-center py-12">
        <CardContent>
          ...
          <p className="text-sm text-burnt-orange mt-2">
            Check back during the MLB season for AI-powered previews and recaps.
          </p>
        </CardContent>
      </Card>
    );
