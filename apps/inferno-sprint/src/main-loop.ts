// Shaders (source code only)
import main_vsSource from './shaders/main-vertex.vert';
import main_fsSource from './shaders/main-fragment.frag';
import collider_fsSource from './shaders/collider-fragment.frag';
import sky_vsSource from './shaders/sky-vertex.vert';
import sky_fsSource from './shaders/sky-fragment.frag';
...
import {
  ...
  uniformName_iResolution,
  CSM_TEXTURE_SIZE,
  CSM_PLANE_DISTANCE,
  COLLISION_TEXTURE_SIZE,
} from './shaders/shader-constants';
...
export const startMainLoop = (groundTextureImage: HTMLImageElement) => {
  ...
  const renderModels = (
    ...
    if (mainMenuVisible) {
      if (hC.width > 1100) {
        ...
        xgl.drawElements(
          xgl.TRIANGLES,
          allModels[MODEL_ID_PLAYER_LEG1]!.$vertexEnd! -
            allModels[MODEL_ID_PLAYER_BODY]!.$vertexBegin!,
          xgl.UNSIGNED_SHORT,
      ...
      xgl.drawElements(
        xgl.TRIANGLES,
        allModels[doNotRenderPlayer ? MODEL_ID_PLAYER_BODY : MODEL_ID_PLAYER_LEG1 + 1]!
          .$vertexBegin! - 3,
        xgl.UNSIGNED_SHORT,
        3 * 2
      );
  ...
  const initShaderProgram = (
    ...
    return (name?: string): any =>
      name
        ? uniforms[name] || (uniforms[name] = xgl.getUniformLocation(program, name)!)
        : xgl.useProgram(program);
  };
  ...
  const [csm0, csm1] = integers_map(2, (split: number) => {
    ...
    return (roundingRadius: number) => {
      ...
      for (let i = 0, j = 0; i < 8; ++i) {
        ...
        tx -= csm_tempFrustumCorners[j++] =
          (matrixTransformPoint.x | 0) / (roundingRadius * matrixTransformPoint.w);
        ty -= csm_tempFrustumCorners[j++] =
          (matrixTransformPoint.y | 0) / (roundingRadius * matrixTransformPoint.w);
        tz -= csm_tempFrustumCorners[j++] =
          (matrixTransformPoint.z | 0) / (roundingRadius * matrixTransformPoint.w);
      }
  ...
  const mainLoop = (globalTime: number) => {
    ...
    if (mainMenuVisible) {
      ...
      matrixToArray(
        tempMatrix,
        transformsBuffer,
        MODEL_ID_PLAYER_BODY - MODELS_WITH_SIMPLE_TRANSFORM - 2
      );
      matrixToArray(
        tempMatrix,
        transformsBuffer,
        MODEL_ID_PLAYER_LEG0 - MODELS_WITH_SIMPLE_TRANSFORM - 2
      );
      matrixToArray(
        tempMatrix,
        transformsBuffer,
        MODEL_ID_PLAYER_LEG1 - MODELS_WITH_SIMPLE_TRANSFORM - 2
      );
    ...
    gl.uniformMatrix4fv(
      ...
      false,
      matrixToArray(camera_view.invertSelf())
    );
    gl.uniform3f(
      skyShader(uniformName_iResolution),
