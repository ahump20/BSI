...
export function debug_camera_load() {
  ...
  const { pos, rot, debugCamera, DEBUG_FLAG0, DEBUG_FLAG1, DEBUG_FLAG2, DEBUG_FLAG3 } =
    savedDebugCameraContent || {};
  if (pos) {
...
export function devInit() {
  ...
  function animationFrame(now: number) {
    ...
    const movForward =
      ...
      (keyboard_downKeys.has('Numpad2') ||
      keyboard_downKeys.has('Numpad5') ||
      keyboard_downKeys.has('KeyJ')
        ? -1
        : 0);
    ...
    if (movStrafe || movForward || movVertical) {
      if (!window.DEBUG_CAMERA) {
        ...
        const movSpeed =
          (keyboard_downKeys.has('ShiftLeft') || keyboard_downKeys.has('ShiftRight') ? 20 : 5.5) *
          deltaTime;
    ...
    if (!window.DEBUG_CAMERA) {
      ...
      set_camera_position(
        debug_camera_position.x,
        debug_camera_position.y,
        debug_camera_position.z
      );
