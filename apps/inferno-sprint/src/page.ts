...
import { mainMenuVisible, setMainMenuVisible, absoluteTime, gameTimeDelta, gameTime } from "./game/game-time";
  ...
  if (b6 && leaderboardOverlay) {
    b6.onclick = async () => {
      try {
        ...
        const data = (await res.json()) as { leaderboard?: Array<{ rank: number; playerName: string; time: number }> };
        ...
        html += '<div class="lb-close"><button onclick="document.getElementById(\'leaderboardOverlay\').classList.remove(\'visible\')">Close</button></div>';

        leaderboardOverlay.innerHTML = html;
        leaderboardOverlay.classList.add("visible");
      } catch (e) {
        leaderboardOverlay.innerHTML = "<h2>Leaderboard</h2><p>Unable to load leaderboard</p><div class='lb-close'><button onclick=\"document.getElementById('leaderboardOverlay').classList.remove('visible')\">Close</button></div>";
        leaderboardOverlay.classList.add("visible");
      }
    };
  }
