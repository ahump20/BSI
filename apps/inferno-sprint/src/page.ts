...
import { camera_rotation, resetGame, getSprintRemaining, formatTime } from './game/world-state';
import {
  mainMenuVisible,
  setMainMenuVisible,
  absoluteTime,
...
import {
  ...
  zFar,
  zNear,
} from './math/matrix-perspective';

// Leaderboard API base URL - uses Worker URL when on Pages, relative path otherwise
const API_BASE = location.hostname.includes('pages.dev')
  ? 'https://bsi-inferno-sprint-leaderboard.humphrey-austin20.workers.dev'
  : '';
...
export let page_update = () => {
  ...
  if (b6 && leaderboardOverlay) {
    b6.onclick = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/leaderboard?limit=25`);
        const data = (await res.json()) as {
          leaderboard?: Array<{ rank: number; playerName: string; time: number }>;
        };
        ...
        if (data.leaderboard && data.leaderboard.length > 0) {
          data.leaderboard.forEach((entry) => {
            ...
            html += `<div class="lb-entry"><span class="lb-rank">#${entry.rank}</span><span class="lb-name">${entry.playerName}</span><span class="lb-time">${timeStr}</span></div>`;
          });
        } else {
          html += '<p>No scores yet. Be the first!</p>';
        }
        html +=
          '<div class="lb-close"><button onclick="document.getElementById(\'leaderboardOverlay\').classList.remove(\'visible\')">Close</button></div>';

        leaderboardOverlay.innerHTML = html;
        leaderboardOverlay.classList.add('visible');
      } catch (e) {
        leaderboardOverlay.innerHTML =
          "<h2>Leaderboard</h2><p>Unable to load leaderboard</p><div class='lb-close'><button onclick=\"document.getElementById('leaderboardOverlay').classList.remove('visible')\">Close</button></div>";
        leaderboardOverlay.classList.add('visible');
      }
    };
  }
  ...
  page_update = () => {
    input_forward =
      touch_movementY +
      (keyboard_downKeys[KEY_FRONT] ? 1 : 0) -
      (keyboard_downKeys[KEY_BACK] ? 1 : 0);
    input_strafe =
      touch_movementX +
      (keyboard_downKeys[KEY_LEFT] ? 1 : 0) -
      (keyboard_downKeys[KEY_RIGHT] ? 1 : 0);

    // Update sprint timer HUD
    if (!mainMenuVisible && timerHud) {
      const remaining = getSprintRemaining();
      timerHud.textContent = formatTime(gameTime);

      // Visual feedback for time pressure
      if (remaining < 15 && remaining > 0) {
        timerHud.className = 'critical';
      } else if (remaining < 30 && remaining > 0) {
        timerHud.className = 'warning';
      } else {
        timerHud.className = '';
      }
    }
