import { COLLISION_TEXTURE_SIZE } from '../shaders/shader-constants';
import {
  max,
  clamp,
  DEG_TO_RAD,
...
export const player_init = NO_INLINE(() => {
  ...
  const doCollisions = NO_INLINE(() => {
    ...
    for (let y = 0; y < LEGS_ROWS; ++y) {
      for (
        let x = 24 * 4, yindex = y * (COLLISION_TEXTURE_SIZE * 4);
        x < (COLLISION_TEXTURE_SIZE - 24) * 4;
        x += 4
      ) {
        for (let k = 0; k < 2; ++k) {
    ...
    currentModelId =
      lineToProcess >= 0 ? (modelBCount > modelACount * 2 ? modelB : currentModelId) : 0;
    ...
    let movZ = 0;
    for (
      let y = LEGS_ROWS, index = COLLISION_TEXTURE_SIZE * LEGS_ROWS * 4;
      y < COLLISION_TEXTURE_SIZE;
      ++y
    ) {
      let left = 0;
      let right = 0;
  ...
  const interpolate_with_hysteresis = (
    previous: number,
    desired: number,
    hysteresis: number,
    speed: number
    ...
    lerp(
      previous,
      desired,
      boot || (clamp(abs(desired - previous) ** 0.5 - hysteresis) + 1 / 7) * damp(speed * 1.5)
    );
  ...
  player_update = () => {
    ...
    if (player_respawned || currentModelId !== oldModelId) {
      ...
      player_position_final.y <
      (player_position_final.x < -20 || player_position_final.z < 109 ? -25 : -9)
    ) {
    ...
    if (currentModelId === MODEL_ID_STATIC_WORLD) {
      levers[LEVER_ID_BOAT1]!.$value =
        player_position_final.x < -15 && player_position_final.z < 0 ? 1 : 0;
    }
    ...
    player_on_rotating_platforms = lerpDamp(
      ...
      shouldRotatePlatforms *
        ((currentModelId > MODEL_ID_ROTATING_PLATFORM0 - 1 &&
          currentModelId < MODEL_ID_ROTATING_PLATFORM0 + 4) as any),
      2
    );

    camera_pos_lookat_x = interpolate_with_hysteresis(
      camera_pos_lookat_x,
      player_position_final.x,
      0.5,
      1
    ...
    camera_pos_lookat_z = interpolate_with_hysteresis(
      camera_pos_lookat_z,
      player_position_final.z,
      0.5,
      1
    ...
    if (!DEBUG_CAMERA) {
      if (player_first_person) {
        ...
        const viewDirDiffz = min(
          CAMERA_PLAYER_Z_DIST / 3,
          -abs(camera_pos_lookat_z - camera_position_z)
        );
        ...
        camera_rotation.x = angle_lerp_degrees(
          camera_rotation.x,
          90 -
            Math.atan2(hypot(viewDirDiffz, viewDirDiffx), camera_position_y - camera_pos_lookat_y) /
              DEG_TO_RAD,
          boot + damp(10)
        );
      }
    ...
    modelsNextUpdate(
      ...
      0.06 *
        ...
        player_legs_speed *
        Math.cos(gameTime * (PLAYER_LEGS_VELOCITY * 2)) +
        player_model_y,
      player_position_final.z
    ).rotateSelf(
      0,
    ...
    player_gravity = currentModelId
      ...
      : lerpDamp(
          player_gravity,
          player_respawned ? 13 : 19 - min(0, player_position_final.y + 10) * 2,
          2.2
        );

    player_fly_velocity_x =
      currentModelId || player_respawned ? 0 : lerpDamp(player_fly_velocity_x, 0, 3);
    player_fly_velocity_z =
      currentModelId || player_respawned ? 0 : lerpDamp(player_fly_velocity_z, 0, 3);
    ...
    movePlayer(
      gameTimeDelta *
        (player_fly_velocity_x + (Math.cos(movAngle) * strafe - Math.sin(movAngle) * forward)),
      gameTimeDelta * -player_gravity,
      gameTimeDelta *
        (player_fly_velocity_z + (Math.sin(movAngle) * strafe + Math.cos(movAngle) * forward))
    );
  };
});
