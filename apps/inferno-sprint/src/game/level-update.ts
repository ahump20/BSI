...
import {
  ...
  verifyModelsNextUpdate,
} from './models-next-update';
import {
  LEVER_ID_GATE0,
  LEVER_ID_GATE1,
...
export let eppur_si_muove = () => {
  ...
  eppur_si_muove = () => {
    ...
    let oscillation = min(
      1 - levers[LEVER_ID_TRIANGLE_PLATFORM]!.$lerpValue2,
      levers[LEVER_ID_GATE1]!.$lerpValue2
    );
    ...
    oscillation =
      ...
      lerpneg(
        levers[LEVER_ID_TRIANGLE_PLATFORM]!.$lerpValue,
        levers[LEVER_ID_ROTATING_CORRIDOR]!.$lerpValue
      );
    ...
    oscillation = lerpneg(
      levers[LEVER_ID_MONUMENT]!.$lerpValue2,
      levers[LEVER_ID_CRYSTALS]!.$lerpValue2
    );
    ...
    modelsNextUpdate(
      ...
      oscillation * Math.sin(gameTime) * 5 +
        3.5 *
          (1 - max(levers[LEVER_ID_CRYSTALS]!.$lerpValue, levers[LEVER_ID_MONUMENT]!.$lerpValue))
    );
    ...
    const hexPadsOscillation = lerpneg(
      levers[LEVER_ID_DETOUR]!.$lerpValue2,
      levers[LEVER_ID_DONUT_PAD]!.$lerpValue2
    );
    ...
    oscillation = lerpneg(
      lerpneg(
        (levers[LEVER_ID_AFTER_JUMPING_PADS]!.$lerpValue +
          levers[LEVER_ID_AFTER_JUMPING_PADS]!.$lerpValue2) /
          2,
        levers[LEVER_ID_AFTER_ROTATING_PLATFORMS]!.$lerpValue2
      ),
      (levers[LEVER_ID_FLOATING_ELEVATOR]!.$lerpValue +
        levers[LEVER_ID_FLOATING_ELEVATOR]!.$lerpValue2) /
        2
    );
    ...
    modelsNextUpdate(
      -75,
      3 *
        (1 - levers[LEVER_ID_ROTATING_CORRIDOR]!.$lerpValue2) *
        (1 - levers[LEVER_ID_CRYSTALS]!.$lerpValue),
      55
    ).rotateSelf(
      180 * (1 - levers[LEVER_ID_ROTATING_CORRIDOR]!.$lerpValue2) + rotatingHexCorridorRotation,
      0
    );
    ...
    modelsNextUpdate(
      ...
      -3 * (1 - levers[LEVER_ID_DETOUR]!.$lerpValue) -
        hexPadsOscillation * Math.sin(gameTime * 0.8) -
        1.8,
      93.5
    ...
    const boatUpdate = (x: number, y: number, z: number) =>
      modelsNextUpdate(
        x + Math.sin(gameTime + 2) / 5,
        y + Math.sin(gameTime * 0.8) / 5,
        z
