...
BSI uses multiple test frameworks for different testing needs. **Note:** `npm test` runs only the Vitest unit/integration tests. For full test coverage, you must run additional test suites separately.

### Running Tests
```bash
# Vitest unit/integration tests only
...
npm run test:validation   # Schema validation tests (vitest)
npm run test:all          # All vitest suites (api + integration + validation)

# Playwright E2E/Accessibility tests (separate from npm test)
npm run test:a11y         # Accessibility tests with axe-core
npm run test:a11y:ui      # Playwright with interactive UI
npm run test:a11y:report  # View Playwright HTML report
```

### Full Test Coverage

To run all test suites for complete coverage before deployment:
```bash
npm run test:all && npm run test:a11y
```
...
├── validation/           # Schema validation (vitest)
