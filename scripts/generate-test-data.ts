...
async function generateWearableDevices(
  db: Client,
  playerIds: string[]
): Promise<Record<string, string>> {
  const devicesMap: Record<string, string> = {};
  const playersWithWearables = playerIds.slice(
    0,
    Math.floor(CONFIG.n_players * CONFIG.wearables_coverage)
  );
...
async function generateWearablesData(
  db: Client,
  devicesMap: Record<string, string>,
  games: any[]
): Promise<void> {
  ...
  for (const game of games) {
    for (let i = 0; i < CONFIG.n_clutch_situations_per_game; i++) {
      ...
      const startTime = randomDate(
        ...
        new Date(game.game_date.getTime() + 3 * 3600000)
      );
...
async function generatePlayerActions(
  db: Client,
  situations: any[],
  playerIds: string[]
): Promise<void> {
  for (const situation of situations) {
    ...
    for (let i = 0; i < nActions; i++) {
      ...
      const isSuccessful =
        actionType.includes('made') || actionType === 'assist' || actionType === 'rebound';
      const pointsScored =
