...
export async function generateLeaderboard(contest, entries, results, options = {}) {
  const { includeStats = true, maxResults = 100, currentUserId = null } = options;
...
function calculatePrizeDistribution(contest, scoredEntries) {
  ...
  const sortedRanks = Object.keys(distributionConfig)
    .map((r) => parseInt(r))
    .sort((a, b) => a - b);
...
function generateLeaderboardStats(scoredEntries, contest, results) {
  ...
  stats.completion_rate = scoredEntries.length > 0 ? completedEntries / scoredEntries.length : 0;
