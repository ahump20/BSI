...
function scorePredictionEntry(entry, results, config) {
  ...
  for (const [gameId, pick] of Object.entries(picks)) {
    ...
    const actualWinner = result.home_score > result.away_score ? result.home_team_id : result.away_team_id;
    ...
    if (predictedWinner === actualWinner) {
      ...
      if (pick.predicted_home_score && pick.predicted_away_score) {
        ...
        else if (Math.abs(predictedHome - actualHome) <= 3 && Math.abs(predictedAway - actualAway) <= 3) {
  ...
  for (const [propId, pick] of Object.entries(picks)) {
    ...
    if (pick.prediction === result.outcome) {
      ...
      breakdown.push({
        prop_id: propId,
        points: propPoints,
        reason: useConfidence
          ? `Correct (confidence ${pick.confidence}x)`
          : 'Correct prediction'
      });
