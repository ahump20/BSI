...
async function fetchTeamComparisonData(widget, env) {
  ...
  const [team1Stats, team2Stats] = await Promise.all([
    ...
    `).bind(sport, team1).first(),
    env.DB.prepare(`
      SELECT * FROM team_stats WHERE sport = ? AND team_id = ?
    `).bind(sport, team2).first()
  ]);
...
async function fetchPredictionsData(widget, env) {
  ...
  return {
    ...
    accuracy: results.results.filter(r => r.correct === 1).length / results.results.length * 100
  };
}
...
export async function deleteDashboard(dashboardId, env) {
  ...
  return { success: true, dashboardId: dashboardId };
}
...
import { getAccuracyMetrics, analyzeCalibration, getAccuracyTrend } from '../ml/prediction-accuracy-tracker.js';
