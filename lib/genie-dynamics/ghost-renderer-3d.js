...
const TRAIL_MATERIAL_NAME = 'genie_trail_mat';
const MAX_GHOST_POOL = 32;
...
export class GhostRenderer3D {
  ...
  _initMaterials() {
    ...
    if (!this.scene.getMaterialByName(GHOST_MATERIAL_NAME)) {
      const mat = new this.scene._PBRMaterial(GHOST_MATERIAL_NAME, this.scene);
  ...
  renderGhosts(entityPredictions, options = {}) {
    ...
    for (const [entityId, data] of entityPredictions) {
      const { predictions, currentPos } = data;
      for (const pred of predictions) {
        ...
        this._placeGhost(worldPos, pred.probability, entityId);
      }
    }
  ...
  renderTrajectoryArc(points, probability, options = {}) {
    ...
    const { color, dashSize = 0.3, gapSize = 0.2 } = options;
  ...
  _placeGhost(worldPos, probability, entityId) {
