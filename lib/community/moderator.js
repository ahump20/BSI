...
export async function reviewFlag(db, flagId, moderatorId, action, notes = '') {
  ...
  if (action === 'remove') {
    ...
    // Warning notifications are sent via the BSI notification queue (Cloudflare Queues)
    // See: lib/notifications/queue.ts for implementation details
  }
...
async function logModeratorAction(db, moderatorId, action, targetId, notes) {
  ...
  `
    ...
    .bind(logId, moderatorId, action, targetId, notes, new Date().toISOString())
    .run();
}
