...
export async function generateCareerScorecard(env, umpireName, sport, season) {
  ...
  `).bind(sport, season, umpireName).all();
...
function analyzeCalls(pitches) {
  ...
  return {
    ...
    correct_strike_calls: correct_strike_calls,
    correct_ball_calls: correct_ball_calls,
    missed_strikes: missed_strikes,
    missed_balls: missed_balls,
    accuracy: Math.round(accuracy * 10) / 10,
    strike_call_accuracy: called_strikes > 0
      ? Math.round((correct_strike_calls / called_strikes) * 1000) / 10
      : 0,
    ball_call_accuracy: called_balls > 0
      ? Math.round((correct_ball_calls / called_balls) * 1000) / 10
      : 0
  };
}
...
function analyzeZones(pitches) {
  ...
  for (const zone in zones) {
    ...
    data.strike_rate = data.total > 0
      ? Math.round((data.called_strikes / data.total) * 1000) / 10
      : 0;
  }
...
function analyzeConsistency(pitches) {
  ...
  const variance = Math.sqrt(
    (Math.pow(firstAccuracy - avgAccuracy, 2) +
     ...
     Math.pow(thirdAccuracy - avgAccuracy, 2)) / 3
  );
