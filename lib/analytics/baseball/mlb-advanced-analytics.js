...
class MLBAdvancedAnalytics {
  ...
  static calculatePythagoreanWins(runsScored, runsAllowed, gamesPlayed) {
    ...
    const pythagWinPct =
      Math.pow(runsScored, exp) / (Math.pow(runsScored, exp) + Math.pow(runsAllowed, exp));
  ...
  static calculateRunDifferential(runsScored, runsAllowed, gamesPlayed) {
    ...
    if (perGameDiff > 1.0)
      color = '#10b981'; // strong positive (green)
    else if (perGameDiff > 0.3)
      color = '#22c55e'; // moderate positive
    else if (perGameDiff < -1.0)
      color = '#ef4444'; // strong negative (red)
    ...
    return {
      ...
      interpretation:
        perGameDiff > 1.0
          ? 'Elite run differential'
          : perGameDiff > 0.3
            ? 'Strong run differential'
            : perGameDiff < -1.0
              ? 'Poor run differential'
              : perGameDiff < -0.3
                ? 'Below average run differential'
  ...
  static calculateStrengthOfSchedule(team, allTeams) {
    ...
    const divisionTeams = allTeams.filter((t) => t.division === team.division && t.id !== team.id);

    if (divisionTeams.length === 0) return 50; // neutral

    const avgWinPct =
      divisionTeams.reduce((sum, t) => sum + (t.percentage || 0), 0) / divisionTeams.length;
  ...
  static generateLeagueAdvancedAnalytics(teams, gamesRemaining = 0) {
    ...
    const leagueTotalRuns = teams.reduce(
      (sum, t) => sum + (t.stats?.runsScored || t.runsScored || 0),
      0
    );
    const leagueTotalGames = teams.reduce((sum, t) => sum + (t.gamesPlayed || 0), 0);
    const leagueAvgRunsPerGame = leagueTotalGames > 0 ? leagueTotalRuns / leagueTotalGames : 0;
