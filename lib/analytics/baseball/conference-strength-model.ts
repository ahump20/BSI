...
export interface TeamRecord {
  ...
  runsScored: number;
  runsAllowed: number;
  opponents: string[]; // Team IDs of opponents played
}

export interface ConferenceStrength {
  conference: string;
  rpi: number;
  sos: number;
  isr: number;
...
export interface RPICalculation {
  teamId: string;
  teamName: string;
  conference: string;
  rpi: number;
...
export interface SOSCalculation {
  teamId: string;
  teamName: string;
  conference: string;
  sos: number;
...
export interface ConferenceComparison {
  ...
  rpiDifference: number;
  sosDifference: number;
  isrDifference: number;
...
export class ConferenceStrengthModel {
   ...
   */
  static calculateConferenceStrength(
    teams: TeamRecord[],
    allTeams?: TeamRecord[]
  ): ConferenceStrength[] {
    ...
    const conferenceStrengths: ConferenceStrength[] = [];
  ...
  static calculateRPI(teams: TeamRecord[]): RPICalculation[] {
    ...
    const owpMap = new Map<string, number>();
    for (const team of teams) {
    ...
    const oowpMap = new Map<string, number>();
    for (const team of teams) {
      if (team.opponents.length === 0) {
   ...
   */
  static compareConferences(
    conference1Teams: TeamRecord[],
    conference2Teams: TeamRecord[],
    allGames: Array<{
      homeTeamId: string;
      awayTeamId: string;
      homeScore: number;
   ...
   */
  private static estimateNCAASeedsForConference(
    teams: TeamRecord[],
    rpiMap: Map<string, RPICalculation>
  ): number {
    let seeds = 0;
   ...
   * (Simplified - production would use historical data and projections)
   */
  private static estimateNCAAAppearancesForConference(
    teams: TeamRecord[],
    rpiMap: Map<string, RPICalculation>
  ): number {
    let appearances = 0;
