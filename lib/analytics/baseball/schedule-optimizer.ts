...
export class ScheduleOptimizer {
  ...
  static runMonteCarloSimulation(
    ...
    // Run simulations
    const simulationResults: number[] = []; // Projected wins for each simulation
   ...
   */
  private static adjustForHomeField(baseProb: number, adjustment: number): number {
   ...
   * Estimate expected run margin
   */
  private static estimateRunMargin(
    teamISR: ISRCalculation,
    oppISR: ISRCalculation,
    location: 'home' | 'away' | 'neutral'
  ): number {
   ...
   */
  private static estimateNCAAProb(wins: number, totalGames: number, rpi: number): number {
   ...
   */
  private static estimateNCAASeedProb(wins: number, totalGames: number, rpi: number): number {
    const winPct = wins / totalGames;
   ...
   */
  private static estimateConferenceChampionshipProb(
  ...
  static generateWhatIfScenarios(
    baseSimulation: MonteCarloSimulation,
    schedule: RemainingSchedule,
    teamMetrics: {
      rpi: RPICalculation;
      sos: SOSCalculation;
  ...
  private static createScenario(
    name: string,
    description: string,
    outcomes: Array<{ gameId: string; assumedResult: 'win' | 'loss' }>,
    schedule: RemainingSchedule,
    teamMetrics: {
      rpi: RPICalculation;
      sos: SOSCalculation;
   ...
   */
  private static generateImportanceReasoning(
    gameProb: { winProbability: number; expectedMargin: number },
    oppMetrics?: {
      rpi: RPICalculation;
      sos: SOSCalculation;
      isr: ISRCalculation;
   ...
   */
  private static generateOptimalOutcomes(
    simulation: MonteCarloSimulation,
    schedule: RemainingSchedule
  ): Array<{
    scenario: string;
