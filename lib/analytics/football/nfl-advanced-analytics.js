...
class NFLAdvancedAnalytics {
  ...
  static calculatePointDifferential(pointsFor, pointsAgainst, gamesPlayed) {
    ...
    if (perGameDiff > 7.0)
      color = '#10b981'; // Elite (>7 PPG) - green
    else if (perGameDiff > 3.0)
      color = '#22c55e'; // Strong (3-7 PPG)
    else if (perGameDiff < -7.0)
      color = '#ef4444'; // Poor (<-7 PPG) - red
  ...
  static calculateStrengthOfSchedule(team, allTeams) {
    ...
    const divisionTeams = allTeams.filter(
      (t) => t.division === team.division && t.conference === team.conference && t.id !== team.id
    );

    if (divisionTeams.length === 0) return { sosRating: 50, interpretation: 'Unknown division' };

    const avgWinPct =
      divisionTeams.reduce((sum, t) => sum + (t.winPercentage || t.percentage || 0), 0) /
      divisionTeams.length;

    // Scale to 0-100 rating (50 = average)
    const sosRating = 50 + (avgWinPct - 0.5) * 100;

    return {
      sosRating: Math.round(sosRating),
      divisionAvgWinPct: avgWinPct,
      interpretation:
        sosRating > 52 ? 'Tough division' : sosRating < 48 ? 'Weak division' : 'Average division',
    };
  }
  ...
  static calculatePlayoffProbability(team, gamesRemaining) {
    const _totalGames = team.gamesPlayed + gamesRemaining;
