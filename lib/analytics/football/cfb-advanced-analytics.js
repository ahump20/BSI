...
class CFBAdvancedAnalytics {
  ...
  static CONFERENCE_POWER_RATINGS = {
    ...
    'Mountain West': 0.5,
    'Sun Belt': 0.45,
    Independent: 0.7, // Average for independents
  };
  ...
  static calculatePythagoreanWins(pointsFor, pointsAgainst, gamesPlayed) {
    ...
    const pythagWinPct =
      Math.pow(pointsFor, exp) / (Math.pow(pointsFor, exp) + Math.pow(pointsAgainst, exp));
  ...
  static calculatePointDifferential(pointsFor, pointsAgainst, gamesPlayed) {
    ...
    if (perGameDiff > 14.0)
      color = '#10b981'; // Elite (>14 PPG) - green
    else if (perGameDiff > 7.0)
      color = '#22c55e'; // Strong (7-14 PPG)
    else if (perGameDiff < -14.0)
      color = '#ef4444'; // Poor (<-14 PPG) - red
    ...
    return {
      ...
      interpretation:
        perGameDiff > 14.0
          ? 'Elite point differential'
          : perGameDiff > 7.0
            ? 'Strong point differential'
            : perGameDiff < -14.0
              ? 'Poor point differential'
              : perGameDiff < -7.0
                ? 'Below average point differential'
  ...
  static calculateStrengthOfSchedule(team, allTeams) {
    ...
    const conferenceRating = this.CONFERENCE_POWER_RATINGS[teamConference] || 0.7;
    ...
    let avgWinPct = 0.5; // Default
    if (conferenceTeams.length > 0) {
      avgWinPct =
        conferenceTeams.reduce((sum, t) => sum + (t.winPercentage || t.percentage || 0), 0) /
    ...
    const sosRating = 50 + (conferenceRating - 0.7) * 50 * 0.7 + (avgWinPct - 0.5) * 100 * 0.3;
    ...
    return {
      ...
      interpretation:
        sosRating > 60
          ? 'Very tough schedule (Power 5 conference)'
          : sosRating > 50
            ? 'Above average schedule'
            : sosRating < 40
              ? 'Weak schedule (Group of 5 conference)'
              : 'Average schedule',
    };
  }
  ...
  static calculatePlayoffProbability(team, gamesRemaining) {
    ...
    const teamConference = team.conference?.name || team.conference || 'Independent';
    const conferencePower = this.CONFERENCE_POWER_RATINGS[teamConference] || 0.7;
  ...
  static generateTeamAnalytics(team, allTeams, gamesRemaining = 0) {
    ...
    if (luckFactor > 1.5)
      luckColor = '#10b981'; // Very lucky (green)
    else if (luckFactor > 0.5)
      luckColor = '#22c55e'; // Lucky
    else if (luckFactor < -1.5)
      luckColor = '#ef4444'; // Very unlucky (red)
    ...
    if (luckFactor > 2) luckInterpretation = 'Significantly overperforming - regression likely';
      ...
      luckInterpretation = 'Significantly underperforming - improvement likely';
    ...
    return {
      ...
      playoffProbability:
        gamesRemaining > 0 ? this.calculatePlayoffProbability(team, gamesRemaining) : null,
      dataQuality: {
  ...
  static generateLeagueAdvancedAnalytics(teams, gamesRemaining = 0) {
    ...
    const leagueTotalPoints = teams.reduce(
      (sum, t) => sum + (t.stats?.pointsFor || t.pointsFor || 0),
      0
    );
    const leagueTotalGames = teams.reduce((sum, t) => sum + (t.gamesPlayed || 0), 0);
    const leagueAvgPointsPerGame = leagueTotalGames > 0 ? leagueTotalPoints / leagueTotalGames : 0;
