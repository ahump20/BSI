...
const stressResilienceScore = (samples: PhysiologicalSample[]) => {
  ...
  const hrvScore = clamp(
    (safeAverage(samples.map((sample) => sample.heartRateVariability)) / 110) * 100
  );
  return clamp(trendScore * 0.55 + hrvScore * 0.45);
};

const expressionFocusScore = (
  snapshots: MicroExpressionSnapshot[],
  situation: MicroExpressionSnapshot['situation']
) => {
  ...
  const weightedIntensity = safeAverage(
    filtered.map(
      (snap) => snap.intensity * snap.confidence * (snap.expression === 'focus' ? 1.1 : 0.9)
    )
  );
...
const expressionAggressionScore = (snapshots: MicroExpressionSnapshot[]) => {
  const aggressive = snapshots.filter((snap) =>
    ['determination', 'anger'].includes(snap.expression)
  );
  if (!aggressive.length) return 45;
  const weighted = safeAverage(
    aggressive.map(
      (snap) => snap.intensity * (snap.expression === 'anger' ? 0.9 : 1.05) * snap.arousal
    )
  );
...
const aggressionConversionScore = (plays: PerformancePlay[]) => {
  ...
  const successRate = safeAverage(
    aggressivePlays.map((play) => (play.outcome === 'success' ? 1 : 0))
  );
...
const clutchPerformanceScore = (plays: PerformancePlay[]) => {
  ...
  const successRate = safeAverage(
    highLeveragePlays.map((play) => (play.outcome === 'success' ? 1 : 0))
  );
...
const flowCoherenceScore = (samples: PhysiologicalSample[]) => {
  if (!samples.length) return 50;
  const hrv = clamp(
    (safeAverage(samples.map((sample) => sample.heartRateVariability)) / 120) * 100
  );
  const respiration = clamp(
    (1 - Math.abs(18 - safeAverage(samples.map((sample) => sample.respirationRate ?? 18))) / 12) *
      100
  );
...
const killerInstinctFromPerformance = (plays: PerformancePlay[]) => {
  const lateAggressive = plays.filter(
    (play) => play.phase === 'late' && play.aggressionFactor >= 0.5
  );
  if (!lateAggressive.length) return 50;
  const successRate = safeAverage(
    lateAggressive.map((play) => (play.outcome === 'success' ? 1 : 0))
  );
...
const recoveryQuicknessScore = (plays: PerformancePlay[]) => {
  const failed = plays.filter(
    (play) => play.outcome === 'failure' && typeof play.recoveryTimeSec === 'number'
  );
...
const winnerDNAScore = (plays: PerformancePlay[], resume?: ResumeSignals) => {
  ...
  const eliminationRate = elimination
    ...
    : 55;
  const seasonWinPct = resume?.seasonWinPct ? rateToScore(resume.seasonWinPct, 0.55, 0.75) : 60;
  const streaks = resume?.consecutiveWins
    ? clamp(40 + Math.log10(Math.max(resume.consecutiveWins, 1)) * 20)
    : 55;
...
const _predatorMindsetScore = (
  ...
  const predExpressions = clamp(
    expressionAggressionScore(expressions) * 0.7 +
      expressionFocusScore(expressions, 'pressure') * 0.3
  );
...
const aggregateDimension = (
  ...
  const score = clamp(normalizedContributions.reduce((acc, item) => acc + item.weightedScore, 0));
  return {
    key,
    score: round(score, 2),
  ...
  static evaluate(input: DiamondCertaintyInput): DiamondCertaintyReport {
      ...
      buildContribution(
        ...
        clamp(
          safeAverage(input.performance.map((play) => play.winProbabilityAdded)) * 1.5 +
            rateToScore(
              safeAverage(input.performance.map((play) => (play.outcome === 'success' ? 1 : 0))),
              0.5,
              0.75
            ) *
              0.5
    ...
    const winner = aggregateDimension('winnerDNA', [
      ...
      buildContribution(
        'context',
        'opponent_adjustment',
        input.context?.opponentStrength ? clamp(50 + input.context.opponentStrength * 40) : 55,
        0.15,
      ...
      buildContribution(
        'performance',
        'series_close_rate',
        input.resume?.seriesCloseRate ? rateToScore(input.resume.seriesCloseRate, 0.5, 0.78) : 58,
        0.15,
