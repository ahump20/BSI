...
function detectSequentialPatterns(decisions) {
  ...
  for (let i = 0; i < decisions.length - minSequenceLength + 1; i++) {
    const sequence = decisions.slice(i, i + minSequenceLength)
...
function detectSituationalPatterns(decisions) {
  ...
  for (const situation in situations) {
    ...
    const mostCommon = Object.entries(data.decisions)
      .sort((a, b) => b[1] - a[1])[0];
    ...
    if (mostCommon && data.total >= 3) {
      ...
      if (percentage > 60) {
        tendencies[situation] = {
          ...
          success_rate: data.outcomes.success + data.outcomes.failure > 0
            ? Math.round((data.outcomes.success / (data.outcomes.success + data.outcomes.failure)) * 100)
            : null
        };
      }
    }
...
function analyzeRiskProfile(decisions) {
  ...
  return {
    ...
    risk_distribution: {
      ...
      medium_risk: Math.round(((decisions.length - aggressiveDecisions - conservativeDecisions) / decisions.length) * 100),
...
function calculatePredictability(decisions) {
  ...
  return {
    ...
    most_common_decision: Object.entries(decisionCounts)
...
function detectAdaptation(decisions) {
  ...
  if (adaptation.detected) {
    adaptation.description = riskChange > 0
...
function detectZonePreferences(scorecards) {
  ...
  for (const zone in aggregateZones) {
    ...
    const strikeRate = data.total_pitches > 0
      ? (data.total_strikes / data.total_pitches) * 100
      : 0;

    const avgDeviation = data.deviations.length > 0
...
function detectStreaks(performances) {
  ...
  for (let i = 0; i < performances.length; i++) {
    ...
    } else if ((currentStreak.type === 'hot' && isGood) || (currentStreak.type === 'cold' && !isGood)) {
      // Continue streak
  ...
  return {
    streaks,
    longest_hot: streaks.filter(s => s.type === 'hot').sort((a, b) => b.length - a.length)[0] || null,
    longest_cold: streaks.filter(s => s.type === 'cold').sort((a, b) => b.length - a.length)[0] || null,
...
function isGoodPerformance(perf) {
  ...
  if (perf.sport === 'MLB' || perf.sport === 'NCAA_BASEBALL') {
    if (perf.batting_average !== undefined) {
      return perf.batting_average > 0.250;
    }
    if (perf.era !== undefined) {
      return perf.era < 4.00;
    }
  }
...
function detectSituationalPerformance(performances) {
  ...
  for (const perf of performances) {
    ...
    // Opponent quality
    if (perf.opponent_win_pct && perf.opponent_win_pct > 0.550) {
      situations.vs_good.performances.push(perf);
    } else if (perf.opponent_win_pct && perf.opponent_win_pct < 0.450) {
...
function detectTimeOfDayPatterns(performances) {
  ...
  const nightAvg = nightGames.reduce((sum, p) => sum + getPerformanceScore(p), 0) / nightGames.length;
