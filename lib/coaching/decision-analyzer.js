...
function isOptimalTwoPointDecision(decision) {
  ...
  if (quarter === 4 && time_remaining < 300) { // Less than 5 minutes
...
function analyzePlayCalling(pbpData) {
  ...
  // Calculate metrics
  analysis.run_pass_ratio = analysis.pass_plays > 0
...
function calculateDecisionSummary(decisions) {
  ...
  const qualityScore = totalDecisions > 0
    ? Math.round((optimalDecisions / totalDecisions) * 100)
    : 0;
...
export async function analyzeSeasonDecisions(env, teamId, sport, season) {
  try {
    ...
    `).bind(teamId, teamId, sport, season).all();
    ...
    for (const game of games.results) {
      ...
      // Aggregate stats
      seasonAnalysis.aggregate.fourth_down.total_attempts += gameAnalysis.decisions.fourth_down.length;
...
function calculateAggregateMetrics(games) {
  ...
  for (const game of games) {
    ...
    if (game.decisions.play_calling.predictability_score) {
      aggregate.play_calling.total_predictability += game.decisions.play_calling.predictability_score;
  ...
  if (aggregate.fourth_down.total_attempts > 0) {
    aggregate.fourth_down.conversion_rate =
      Math.round((aggregate.fourth_down.successes / aggregate.fourth_down.total_attempts) * 100);
    aggregate.fourth_down.optimal_rate =
      Math.round((aggregate.fourth_down.optimal_decisions / aggregate.fourth_down.total_attempts) * 100);
  }

  if (aggregate.two_point.total_attempts > 0) {
    aggregate.two_point.success_rate =
      Math.round((aggregate.two_point.successes / aggregate.two_point.total_attempts) * 100);
    aggregate.two_point.optimal_rate =
      Math.round((aggregate.two_point.optimal_decisions / aggregate.two_point.total_attempts) * 100);
  }

  if (aggregate.play_calling.games_analyzed > 0) {
    aggregate.play_calling.avg_predictability =
      Math.round(aggregate.play_calling.total_predictability / aggregate.play_calling.games_analyzed);
  }

  // Overall decision quality
  const totalDecisions = aggregate.fourth_down.total_attempts + aggregate.two_point.total_attempts;
  const optimalDecisions = aggregate.fourth_down.optimal_decisions + aggregate.two_point.optimal_decisions;

  aggregate.decision_quality_score = totalDecisions > 0
    ? Math.round((optimalDecisions / totalDecisions) * 100)
    : 0;
