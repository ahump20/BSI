class BlazeCopilotWidget {
  ...
  async fetchResults(query, sport) {
    ...
    try {
      ...
      this.setStatus(
        `Showing ${data.resultsCount ?? (data.results ? data.results.length : 0)} result(s).`,
        false
      );
    } catch (error) {
      ...
      this.setStatus(
        'Unable to retrieve Copilot results right now. Please try again shortly.',
        true
      );
  ...
  renderResults(data) {
    ...
    results.forEach((result) => {
      ...
      const date = result.game_date
        ? new Date(result.game_date).toLocaleDateString(undefined, {
            year: 'numeric',
            month: 'short',
            day: 'numeric',
          })
  ...
  renderEmptyState() {
    ...
    message.textContent =
      'No Copilot insights yet. Try asking about a team, matchup, or performance trend.';
