...
function analyzeMoneyline(modelProb, moneyline) {
  ...
  const homeKelly =
    homeEdge > 0
      ? fractionalKelly *
        ((modelProb.homeWinProbability * (moneyline.home / 100 + 1) - 1) / (moneyline.home / 100))
      : 0;
  const awayKelly =
    awayEdge > 0
      ? fractionalKelly *
        ((modelProb.awayWinProbability * (Math.abs(moneyline.away) / 100 + 1) - 1) /
          (Math.abs(moneyline.away) / 100))
      : 0;
...
function analyzeSpread(modelProb, spread, sport) {
  ...
  const _isFavorite = spreadLine < 0;
  const _spreadMagnitude = Math.abs(spreadLine);
  ...
  switch (sport.toUpperCase()) {
    ...
    case 'NCAA_BASEBALL':
      ...
      spreadAdjustment = spreadLine * 0.2;
      break;
    case 'NBA':
  ...
  const homeSpreadProb = Math.max(0, Math.min(1, modelProb.homeWinProbability + spreadAdjustment));
...
async function analyzeTotal(gameState, total, env) {
  ...
  const [homeOffense, homeDefense, awayOffense, awayDefense] = await Promise.all([
    ...
    getTeamOffensiveRating(env, gameState.awayTeam, gameState.sport),
    getTeamDefensiveRating(env, gameState.awayTeam, gameState.sport),
  ]);
...
function identifyBestOpportunity(analyses) {
  ...
  return {
    ...
    confidence: maxEdge > 0.1 ? 'high' : maxEdge > 0.05 ? 'medium' : 'low',
  };
}
...
function analyzeLineMovement(lineHistory, betType) {
  ...
  switch (betType) {
    case 'moneyline':
      ...
      const _awayMove = current.away - initial.away;
...
function detectSharpMoney(lineHistory) {
  ...
  if (lineHistory.moneyline && lineHistory.moneyline.length >= 3) {
    ...
    const avgMove =
      recent.reduce(
        (sum, line, i) => (i === 0 ? 0 : sum + Math.abs(line.home - recent[i - 1].home)),
        0
      ) / 2;

    if (avgMove > 15) {
      // 15+ cent move is significant
      signals.push({
        type: 'steam_move',
        betType: 'moneyline',
        magnitude: avgMove,
  ...
  if (lineHistory.spread && lineHistory.spread.length >= 3) {
    ...
    const avgMove =
      recent.reduce(
        (sum, line, i) => (i === 0 ? 0 : sum + Math.abs(line.line - recent[i - 1].line)),
        0
      ) / 2;

    if (avgMove > 1) {
      // 1+ point move is significant
      signals.push({
        type: 'sharp_spread',
        betType: 'spread',
        magnitude: avgMove,
...
async function getTeamOffensiveRating(env, teamId, sport) {
  try {
    ...
    `
      ...
      .bind(teamId, teamId, teamId, sport)
      .first();
