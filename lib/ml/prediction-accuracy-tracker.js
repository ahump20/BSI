...
export async function updateActualOutcome(gameId, actualOutcome, env) {
  try {
    ...
    const brierScore = calculateBrierScore(predictionRecord.predicted_home_win_prob, actualHomeWin);
...
export async function analyzeCalibration(env, sport = null) {
  try {
    ...
    const overallCalibrationError =
      bucketsWithData > 0 ? (totalCalibrationError / bucketsWithData) * 100 : 0;
    ...
    return {
      ...
      calibrationQuality:
        overallCalibrationError < 5
          ? 'excellent'
          : overallCalibrationError < 10
            ? 'good'
            : overallCalibrationError < 15
              ? 'fair'
              : 'poor',
