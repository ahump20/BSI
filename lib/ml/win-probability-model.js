...
export async function calculateWinProbability(gameState, env) {
  ...
  try {
    ...
    if (!sport || !homeTeam || !awayTeam || period === undefined || homeScore === undefined || awayScore === undefined) {
...
async function calculateFootballWinProbability(gameState, env) {
  const { homeTeam, awayTeam, quarter, timeRemaining, homeScore, awayScore, possession, down, distance, yardLine } = gameState;
...
async function calculateBaseballWinProbability(gameState, env) {
  const { homeTeam, awayTeam, inning, isTopHalf, homeScore, awayScore, outs, runnersOn } = gameState;
...
async function calculateBasketballWinProbability(gameState, env) {
  const { homeTeam, awayTeam, quarter, timeRemaining, homeScore, awayScore, possession } = gameState;
  ...
  return {
    ...
    factors: {
      ...
      gameState: {
        ...
        possessionsRemaining: possessionsRemaining,
  ...
  try {
    ...
    for (const play of plays) {
      ...
      trend.push({
        ...
        probSwing: trend.length > 0 ?
          Math.round((winProb.homeWinProbability - trend[trend.length - 1].homeWinProb) * 100) / 100 : 0
      });
    }
