...
import { predictInjuryImpact } from './injury-impact-predictor.js';
  ...
  try {
    ...
    for (const game of games.results) {
      ...
      const testPoints = [
        ...
        { label: 'Mid', period: 2, timeRemaining: 450, homeScore: Math.floor(game.home_score * 0.4), awayScore: Math.floor(game.away_score * 0.4) },
        { label: 'Late', period: 4, timeRemaining: 180, homeScore: Math.floor(game.home_score * 0.9), awayScore: Math.floor(game.away_score * 0.9) }
      ];
    ...
    for (const point of ['Start', 'Mid', 'Late']) {
      ...
      const pointBrier = pointResults.reduce((sum, r) => sum + r.brierScore, 0) / pointResults.length;
...
export async function crossValidateModel(env, sport, folds = 5) {
  try {
    ...
    if (games.results.length < folds * 10) {
      throw new Error(`Not enough data for ${folds}-fold cross-validation. Need at least ${folds * 10} games.`);
    }
    ...
    console.log(`Cross-validation complete. Avg accuracy: ${avgAccuracy.toFixed(1)}% ± ${stdDevAccuracy.toFixed(1)}%`);
...
export async function benchmarkPerformance(env) {
  try {
    ...
    console.log(`Win Probability: Avg ${benchmarks.winProbability.avgLatency}ms, P95 ${benchmarks.winProbability.p95Latency}ms`);
    ...
    console.log(`Betting Analysis: Avg ${benchmarks.bettingAnalysis.avgLatency}ms, P95 ${benchmarks.bettingAnalysis.p95Latency}ms`);

    // Overall metrics
    const overallLatency = (benchmarks.winProbability.avgLatency + benchmarks.bettingAnalysis.avgLatency) / 2;
    const performanceGrade = overallLatency < 50 ? 'A' :
                            overallLatency < 100 ? 'B' :
                            overallLatency < 200 ? 'C' : 'D';
...
export async function testEdgeCases(env) {
  try {
    ...
    const edgeCases = [
      {
        name: 'Blowout Early',
        gameState: { sport: 'NFL', homeTeam: 'KC', awayTeam: 'BUF', homeScore: 35, awayScore: 0, quarter: 2, timeRemaining: 600 },
        expectedHomeWinProb: '> 0.95'
      },
      {
        name: 'Tie Game Final Seconds',
        gameState: { sport: 'NFL', homeTeam: 'KC', awayTeam: 'BUF', homeScore: 21, awayScore: 21, quarter: 4, timeRemaining: 10 },
        expectedHomeWinProb: '~0.50'
      },
      {
        name: 'Home Possession Field Goal Range',
        gameState: { sport: 'NFL', homeTeam: 'KC', awayTeam: 'BUF', homeScore: 21, awayScore: 21, quarter: 4, timeRemaining: 30, possession: 'home', yardLine: 30 },
        expectedHomeWinProb: '> 0.70'
      },
      {
        name: 'Late Comeback Scenario',
        gameState: { sport: 'NFL', homeTeam: 'KC', awayTeam: 'BUF', homeScore: 14, awayScore: 21, quarter: 4, timeRemaining: 120, possession: 'home' },
        expectedHomeWinProb: '0.30-0.40'
      },
      {
        name: 'Overtime',
        gameState: { sport: 'NFL', homeTeam: 'KC', awayTeam: 'BUF', homeScore: 24, awayScore: 24, quarter: 5, timeRemaining: 600, possession: 'home' },
    ...
    for (const testCase of edgeCases) {
      ...
      console.log(`${testCase.name}: ${(prediction.homeWinProbability * 100).toFixed(1)}% (Expected: ${testCase.expectedHomeWinProb})`);
    }
