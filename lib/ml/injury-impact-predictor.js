...
export async function predictInjuryImpact(injury, env) {
  ...
  try {
    ...
    const qualityDropoff = calculateQualityDropoff(
      playerStats,
      replacementQuality,
      positionalValue
    );
      ...
      historical: {
        ...
        avgImpact:
          comparableInjuries.length > 0
            ? comparableInjuries.reduce((sum, inj) => sum + inj.impact, 0) /
              comparableInjuries.length
            : null,
      },
      confidence: {
...
function getReplacementLevelByPosition(position, sport) {
  ...
  const replacementLevels = {
    NFL: {
      ...
      RB: 0.5, // RB depth - smaller dropoff
      WR: 0.48,
      TE: 0.45,
      OL: 0.52,
      DE: 0.48,
    ...
    MLB: {
      SP: 0.4, // Starting pitcher scarcity
      RP: 0.48,
...
function getPositionalValue(position, sport) {
  ...
  const positionalValues = {
    ...
    MLB: {
      SP: 0.9, // Starting pitcher highest impact
      C: 0.72,
      SS: 0.68,
      CF: 0.65,
      '3B': 0.62,
  ...
  return sportValues[position] || 0.6; // Default positional value
}
...
function calculateWinProbabilityImpact(qualityDropoff, severity, sport) {
  ...
  let baseImpact = qualityDropoff * 0.1; // 10% scaling factor
...
async function getComparableInjuries(env, position, severity, sport) {
  try {
    ...
    `
      ...
      .bind(position, severity, sport, new Date().getFullYear() - 5)
      .all();
...
export async function analyzeTeamInjuries(teamId, sport, env) {
  try {
    ...
    return {
      ...
      mostImpactful: injuryAnalyses.reduce(
        (max, inj) =>
  ...
  try {
    ...
    return {
      ...
      comparison: {
        ...
        advantage:
          Math.abs(healthAdvantage) < 2
            ? 'minimal'
            : Math.abs(healthAdvantage) < 5
              ? 'moderate'
              : 'significant',
        description:
          healthAdvantage < -5
...
export async function trackRecoveryTimeline(playerId, env) {
  try {
    ...
    const daysUntilReturn = expectedReturn
      ? Math.floor((expectedReturn - today) / (1000 * 60 * 60 * 24))
      : null;

    // Recovery phases
    const phases = {
      minor: [
        'Initial treatment (0-2 days)',
        'Light activity (2-5 days)',
        'Full practice (5-7 days)',
      ],
      moderate: [
        'Initial treatment (0-5 days)',
        'Rehab (5-14 days)',
        'Limited practice (14-21 days)',
        'Full return (21+ days)',
      ],
      major: [
        'Surgery/Treatment (0-7 days)',
        'Early rehab (7-30 days)',
        'Progressive rehab (30-60 days)',
        'Return conditioning (60-90 days)',
      ],
      season_ending: [
        'Surgery/Treatment',
        'Recovery period',
    ...
    const currentPhase = Math.min(
      recoveryPhases.length - 1,
      Math.floor(daysSinceInjury / (daysUntilReturn / recoveryPhases.length))
    );
    ...
    return {
      ...
      timeline: {
        ...
        progress: daysUntilReturn
          ? Math.round((daysSinceInjury / (daysSinceInjury + daysUntilReturn)) * 100)
          : null,
      },
      lastUpdated: new Date().toISOString(),
      timezone: 'America/Chicago',
