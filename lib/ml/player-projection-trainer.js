...
function prepareFeatures(data, sport) {
  ...
  for (const player of data) {
    ...
    const _outcome = player.career_outcome ? JSON.parse(player.career_outcome) : {};
...
function extractFootballFeatures(stats, position) {
  return [stats.yards || 500, stats.touchdowns || 3, stats.yards_per_game || 50, stats.games || 10];
}
...
async function trainModel(features, labels, sport) {
  ...
  for (let epoch = 0; epoch < epochs; epoch++) {
    ...
    if (epoch % 20 === 0) {
      console.log(
        `Epoch ${epoch}: Mean Absolute Error = ${(totalError / features.length).toFixed(4)}`
      );
    }
  }
...
async function deprecateOldModels(db, newModelId, sport, modelType) {
  ...
  `
    ...
    .bind(Math.floor(Date.now() / 1000), sport, modelType, newModelId)
    .run();
}
...
async function createTrainingJob(db, jobId, sport, modelType) {
  ...
  `
    ...
    .bind(jobId, modelType, sport, 'running', Math.floor(Date.now() / 1000))
    .run();
}
...
async function updateTrainingJob(db, jobId, status, metrics, duration = null) {
  ...
  `
    ...
    .bind(status, JSON.stringify(metrics), duration, Math.floor(Date.now() / 1000), jobId)
    .run();
}
