  ...
  for (let i = 1; i < history.length; i++) {
    ...
    if (swing >= 0.05) {
      keyMoments.push({
        ...
        magnitude: swing >= 0.10 ? 'major' : swing >= 0.07 ? 'significant' : 'notable'
      });
    }
  }
...
export async function getPredictionMetrics(gameId, env) {
  try {
    ...
    const avgConfidence = history.reduce((sum, snap) => {
    ...
    return {
      ...
      metrics: {
        ...
        largestSwing: keyMoments.length > 0 ?
          Math.max(...keyMoments.map(m => m.swing)) : 0,
