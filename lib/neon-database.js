...
const getProcessEnv = (key) => (typeof process !== 'undefined' ? process.env?.[key] : undefined);

class NeonDatabaseManager {
  constructor(options = {}) {
    this.connectionString = options.connectionString || getProcessEnv('NEON_DATABASE_URL');
    this.pool = null;
    this.options = {
  ...
  async initialize() {
    ...
    try {
      ...
      const _result = await client.query('SELECT version()');
      client.release();
  ...
  async query(text, params = []) {
    ...
    try {
      ...
      // Query completed in ${_duration}ms, ${result.rowCount} rows
      return result;
    } catch (error) {
   ...
   */
  async cleanupCache() {
    const result = await this.query('DELETE FROM api_cache WHERE expires_at < CURRENT_TIMESTAMP');
...
// CLI usage (guarded for Workers compatibility)
if (
  typeof process !== 'undefined' &&
  process.argv && // @workers-compat-ignore - inside typeof guard
  import.meta.url === `file://${process.argv[1]}` // @workers-compat-ignore - inside typeof guard
) {
