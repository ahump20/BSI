...
export async function storePBPData(env, gameId, sport, season, pbpData) {
  try {
    ...
    console.log(`✅ Stored PBP for ${gameId} in R2: ${r2Key} (${formatBytes(compressedData.byteLength)})`);
...
export async function getStorageStats(env, sport = null, season = null) {
  let query = 'SELECT sport, season, COUNT(*) as game_count, SUM(compressed_size) as total_size FROM pbp_storage_refs';
  const params = [];
