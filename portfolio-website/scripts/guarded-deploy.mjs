...
const PROJECT_NAME = 'austin-humphrey-professional-resume-portfolio';
...
async function runSmoke({ previewUrl, previewLabel, productionUrl, productionLabel, headed, artifactDir }) {
  const args = ['run', 'smoke:predeploy', '--', '--preview-url', previewUrl, '--preview-label', previewLabel];

  if (productionUrl) {
    args.push('--production-url', productionUrl, '--production-label', productionLabel);
  }
  ...
  await runCommand(NPM, args, 'smoke:predeploy');
}

async function runGuardedDeploy(args, summary) {
  logStep('Syncing BSI stats');
  await runCommand(NPM, ['run', 'sync-stats'], 'npm run sync-stats');

  logStep('Building portfolio');
  ...
  logStep('Running gated smoke checks against preview and current production');
